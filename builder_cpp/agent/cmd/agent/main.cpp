...
void checkForUpdate(const std::string &uuid, const std::string &token)
{
    ...
    logf("Download URL components - path: '%s', query: '%s'", path.c_str(), query.c_str());
    logf("Download fullPath length: %zu", fullPath.length());
    ...
    // For HTTPS with self-signed certificates, disable certificate validation
    if (port == 443)
    {
    ...
    for (;;)
    {
        ...
        std::string body = "{"
                           ...
                           jsonEscape(machineUID) + "\","
                                                    "\"name_pc\":\"" +
                           jsonEscape(std::string(hostname)) + "\","
                                                               "\"exe_version\":\"" +
                           jsonEscape(buildSlug) + "\","
                                                   "\"system\":\"" +
                           jsonEscape(osVersion) + "\","
                                                   "\"user_name\":\"" +
                           jsonEscape(telemetry.userName) + "\","
                                                            "\"ip_addr\":\"" +
                           jsonEscape(telemetry.ipAddr) + "\","
                                                          "\"external_ip\":\"" +
                           jsonEscape(telemetry.externalIP) + "\","
                                                              "\"total_memory\":" +
