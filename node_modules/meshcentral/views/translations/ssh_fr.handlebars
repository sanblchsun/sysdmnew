<!DOCTYPE html><html dir="ltr" xmlns="http://www.w3.org/1999/xhtml"><head lang="en">
    ...
    <div id="p11" class="noselect" style="overflow:hidden">
        <div id="deskarea0" style="position:relative">
            <div id="deskarea1" class="areaHead">
                ...
                <div>
                    <input id="ConnectButton" style="display:none" type="button" value="Se connecter" onclick="connectButton()">
                    <input id="DisconnectButton" type="button" value="Déconnecter" onclick="connectButton()">
    ...
    <script>
        ...
        var StatusStrs = ["Débranché", "En cours de connexion ...", "Traitement...", "Connecté"];
        var state = 0;
        var socket = null;
        var user = '';
        var pass = '';
        ...
        function sshTunnelAuthDialog(j, func) {
            var x = '';
            if (j.askkeypass) {
                x += addHtmlValue("Authentification", '<select id=dp2authmethod style=width:230px onchange=sshAuthUpdate(event)><option value=3 selected>' + "Clé enregistrée" + '</option><option value=1>' + "Identifiant & Mot de passe" + '</option><option value=2>' + "Identifiant & Clé" + '</option></select>');
            } else {
                x += addHtmlValue("Authentification", '<select id=dp2authmethod style=width:230px onchange=sshAuthUpdate(event)><option value=1 selected>' + "Identifiant & Mot de passe" + '</option><option value=2>' + "Identifiant & Clé" + '</option></select>');
            }
            x += '<div id=d2userauth style=display:none>';
            x += addHtmlValue("Nom d'utilisateur", '<input id=dp2user style=width:230px maxlength=64 autocomplete=off onkeyup=sshAuthUpdate(event) />');
            x += '</div>';
            x += '<div id=d2passauth style=display:none>';
            x += addHtmlValue("Mot de passe", '<input type=password id=dp2pass style=width:230px maxlength=64 autocomplete=off onkeyup=sshAuthUpdate(event) />');
            if ((features & 1) == 0) { x += addHtmlValue('', '<label><input id=dp2keep type=checkbox>' + "Mémoriser les identifiants" + '</label>'); }
            x += '</div><div id=d2keyauth style=display:none>';
            x += addHtmlValue("Fichier de la clé", '<input type=file id=dp2key style=width:230px maxlength=64 autocomplete=off onchange=sshAuthUpdate(event) />' + '<div id=d2badkey style=font-size:x-small>' + "La clé doit être au format OpenSSH" + '</div>');
            x += addHtmlValue("Mot de passe de la clé", '<input type=password id=dp2keypass style=width:230px maxlength=64 autocomplete=off onkeyup=sshAuthUpdate(event) />');
            if ((features & 1) == 0) {
                x += addHtmlValue('', '<label><input id=dp2keep1 type=checkbox onchange=sshAuthUpdate(event)>' + "Se souvenir de l'utilisateur et de la clé" + '</label>');
                x += addHtmlValue('', '<label><input id=dp2keep2 type=checkbox>' + "Se souvenir du mot de passe" + '</label>');
            }
            x += '</div>';
            if (j.askkeypass) {
                x += '<div id=d2keyauth2 style=display:none>';
                x += addHtmlValue("Mot de passe", '<input type=password id=dp2keypass2 style=width:230px maxlength=64 autocomplete=off onkeyup=sshAuthUpdate(event) />');
                x += '</div>';
            }
            ...
            socket.onmessage = function (data) {
                ...
                if (data.data[0] == '{') {
                    ...
                    switch (json.action) {
                        ...
                        case 'autherror': { setDialogMode(2, "Authentification", 1, null, "Authentification impossible."); break; }
                        case 'sessionerror': { setDialogMode(2, "Session", 1, null, "Session expirée."); break; }
                        case 'sessiontimeout': { setDialogMode(2, "Session", 1, null, "Durée de la session."); break; }
                    }
                } else if (data.data[0] == '~') {
