        ...
        <div id="deskarea0">
            <div id="deskarea1" class="areaHead">
                <div class="toright2">
                    ...
                    <input id="ConvertAsWebM" style="display:none" type="button" value="Pretvorite u WebM" onclick="saveAsWebMfile()">&nbsp;
                </div>
                <div>
                    <input id="OpenFileButton" type="button" value="Otvori datoteku..." onclick="openfile()" style="display:none">
            ...
            <div id="deskarea4" class="areaFoot">
                ...
                <div>
                    &nbsp;
                    <input id="PlayButton" type="button" value="Igraj" disabled="disabled" onclick="play()">
                    <input id="PauseButton" type="button" value="Pauza" disabled="disabled" onclick="pause()">
                    <input id="RestartButton" type="button" value="Ponovo pokreni" disabled="disabled" onclick="restart()">
                    <select id="PlaySpeed" onchange="this.blur();">
                        <option value="4">1/4 Brzina</option>
                        <option value="2">1/2 Brzina</option>
                        <option value="1" selected="">Normalna brzina</option>
                        <option value="0.5">2x Brzina</option>
                        <option value="0.25">4x Brzina</option>
                        <option value="0.1">10x Brzina</option>
                    </select>
    ...
    <script>
        ...
        function start() {
            ...
            browser = (function (agent) {
                switch (true) {
                    case agent.indexOf("rub") > -1: return "MS Edge (EdgeHtml)";
                    case agent.indexOf("edg") > -1: return "MS Edge Chromium";
                    case agent.indexOf("opr") > -1 && !!window.opr: return "opera";
                    case agent.indexOf("hrom") > -1 && !!window.chrome: return "hrom";
            ...
            if (urlargs.stream != null) {
                QV('metadatadiv', true);
                QH('metadatadiv', "Spajanje na server...");
        ...
        function processFirstBlock(type, flags, time, data) {
            ...
            var x = '';
            x += addInfo("Vrijeme", recFileMetadata.time);
            if (recFileEndTime != 0) { var secs = Math.floor((recFileEndTime - time) / 1000); x += addInfo("Trajanje", format("{0} sekunda{1}", secs, (secs > 1) ? 's' : '')); }
            x += addInfo("Korisničko ime", recFileMetadata.username);
            x += addInfo("Korisnički broj", recFileMetadata.userid);
            x += addInfo("ID sesije", recFileMetadata.sessionid);
            if (recFileMetadata.ipaddr1 && recFileMetadata.ipaddr2) { x += addInfo("Adrese", format("{0} do {1}", recFileMetadata.ipaddr1, recFileMetadata.ipaddr2)); }
            if (recFileMetadata.devicename) { x += addInfo("Naziv uređaja", recFileMetadata.devicename); }
            ...
            if (recFileMetadata.protocol) {
                ...
                else if (p == 8) { p = "Korisnička školjka"; }
                else if (p == 9) { p = "Korisnik PowerShell"; }
                else if (p == 100) { p = "Intel&reg; AMT WSMAN"; }
                else if (p == 101) { p = "Intel&reg; AMT preusmjeravanje"; }
                else if ((p == 102) || (p == 200 && recFileMetadata.bpp != null)) { p = "Intel&reg; AMT KVM"; }
                else if (p == 200) { p = "MeshMessenger"; }
                x += addInfoNoEsc("Protokol", p);
            }
            var encQualityStr = "2 bajta po pikselu";
            if (recFileMetadata.bpp == 1) { encQualityStr = "1 bajt po pikselu" }
            if (recFileMetadata.graymode) { if (recFileMetadata.lowcolor) { encQualityStr += ", 16 siva"; } else { encQualityStr += ", 256 sivih"; } }
            x += addInfoNoEsc("Kvalitet kodiranja", encQualityStr);

            if (recFileMetadata.indexInterval) {
                recFileIndexBasePtr = recFilePtr;
                x += addInfoNoEsc("Traženje", format("Indeksira se svakih {0} sekundi", recFileMetadata.indexInterval));
            ...
            if ((recFileMetadata.protocol == 1) || (recFileMetadata.protocol == 6) || (recFileMetadata.protocol == 8) || (recFileMetadata.protocol == 9)) {
                ...
                recFileProtocol = 1;
                x += '<br /><br /><span style=color:gray>' + "Pritisnite [razmak] za reprodukciju/pauziranje." + '</span>';
                QE('PlayButton', true);
        ...
        function cleanup() {
            ...
            QV('metadatadiv', true);
            if (urlargs.stream == null) {
                QH('metadatadiv', '<span style=\"font-family:Arial,Helvetica Neue,Helvetica,sans-serif;font-size:28px\">MeshCentral Session Player</span><br /><br /><span style=color:gray>' + "Prevucite i ispustite datoteku .mcrec ili kliknite na \"Otvori datoteku...\"" + '</span>');
            } else {
                QH('metadatadiv', '');
            }
            QV('DeskParent', true);
        ...
        function saveAsWebMfile() {
            var x = '';
            x += addHtmlValue4("Brzina kadrova", '<select id=webmframerate style=width:200px><option value=100 selected>' + "10 kadrova/sek" + '</option><option value=50>' + "20 kadrova/sek" + '</option></select>');
            x += addHtmlValue4("Kvaliteta", '<select id=webmquality style=width:200px><option value=90>' + "90%" + '</option><option value=80>' + "80%" + '</option><option value=60 selected>' + "60%" + '</option><option value=40>' + "40%" + '</option><option value=20>' + "20%" + '</option><option value=10>' + "10%" + '</option></select>');
            setDialogMode(2, "Pretvorite u WebM", 3, saveAsWebMfileEx, x);
        }
        ...
        function openfile() {
            ...
            setDialogMode(2, "Otvori datoteku...", 3, openfileEx, x);
