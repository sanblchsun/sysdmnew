<!DOCTYPE html><html dir="ltr" xmlns="http://www.w3.org/1999/xhtml"><head lang="en">
    ...
    <div id="p11" class="noselect" style="overflow:hidden">
        <div id="deskarea0" style="position:relative">
            <div id="deskarea1" class="areaHead">
                ...
                <div>
                    <input id="ConnectButton" style="display:none" type="button" value="Připojit" onclick="connectButton()">
                    <input id="DisconnectButton" type="button" value="Odpojit" onclick="connectButton()">
    ...
    <script>
        ...
        var StatusStrs = ["Odpojeno", "Připojování…", "Nastavení…", "Připojeno"];
        var state = 0;
        var socket = null;
        var user = '';
        var pass = '';
        ...
        function sshTunnelAuthDialog(j, func) {
            var x = '';
            if (j.askkeypass) {
                x += addHtmlValue("Ověření", '<select id=dp2authmethod style=width:230px onchange=sshAuthUpdate(event)><option value=3 selected>' + "Uložený klíč" + '</option><option value=1>' + "Uživatelské jméno heslo" + '</option><option value=2>' + "Uživatelské jméno a klíč" + '</option></select>');
            } else {
                x += addHtmlValue("Ověření", '<select id=dp2authmethod style=width:230px onchange=sshAuthUpdate(event)><option value=1 selected>' + "Uživatelské jméno heslo" + '</option><option value=2>' + "Uživatelské jméno a klíč" + '</option></select>');
            }
            x += '<div id=d2userauth style=display:none>';
            x += addHtmlValue("Uživatelské jméno", '<input id=dp2user style=width:230px maxlength=64 autocomplete=off onkeyup=sshAuthUpdate(event) />');
            x += '</div>';
            x += '<div id=d2passauth style=display:none>';
            x += addHtmlValue("Heslo", '<input type=password id=dp2pass style=width:230px maxlength=64 autocomplete=off onkeyup=sshAuthUpdate(event) />');
            if ((features & 1) == 0) { x += addHtmlValue('', '<label><input id=dp2keep type=checkbox>' + "Pamatujte si přihlašovací údaje" + '</label>'); }
            x += '</div><div id=d2keyauth style=display:none>';
            x += addHtmlValue("Soubor klíče", '<input type=file id=dp2key style=width:230px maxlength=64 autocomplete=off onchange=sshAuthUpdate(event) />' + '<div id=d2badkey style=font-size:x-small>' + "Soubor klíče musí být ve formátu OpenSSH." + '</div>');
            x += addHtmlValue("Klíčové heslo", '<input type=password id=dp2keypass style=width:230px maxlength=64 autocomplete=off onkeyup=sshAuthUpdate(event) />');
            if ((features & 1) == 0) {
                x += addHtmlValue('', '<label><input id=dp2keep1 type=checkbox onchange=sshAuthUpdate(event)>' + "Zapamatujte si uživatele a klíč" + '</label>');
                x += addHtmlValue('', '<label><input id=dp2keep2 type=checkbox>' + "Pamatuj si heslo" + '</label>');
            }
            x += '</div>';
            if (j.askkeypass) {
                x += '<div id=d2keyauth2 style=display:none>';
                x += addHtmlValue("Heslo", '<input type=password id=dp2keypass2 style=width:230px maxlength=64 autocomplete=off onkeyup=sshAuthUpdate(event) />');
                x += '</div>';
            }
            ...
            socket.onmessage = function (data) {
                ...
                if (data.data[0] == '{') {
                    ...
                    switch (json.action) {
                        ...
                        case 'autherror': { setDialogMode(2, "Ověření", 1, null, "Nelze ověřit."); break; }
                        case 'sessionerror': { setDialogMode(2, "Zasedání", 1, null, "Relace vypršela."); break; }
                        case 'sessiontimeout': { setDialogMode(2, "Zasedání", 1, null, "Časový limit relace."); break; }
                    }
                } else if (data.data[0] == '~') {
