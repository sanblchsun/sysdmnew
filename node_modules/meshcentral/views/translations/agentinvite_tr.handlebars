<!DOCTYPE html><html lang="tr"><head>
        ...
        <div id="column_l" style="overflow-y:auto">
            ...
            <div>
                <div class="tab">
                    ...
                    <button id="tlinuxbinarytab" class="tablinks" onclick="openTab(event, 'linuxbinarytab')">Linux / BSD / macOS İkili Yükleyici</button>
                    ...
                    <button id="tassistab" class="tablinks" onclick="openTab(event, 'assistab')">Asistan</button>
                </div>

                <div id="wintab64" class="tabcontent" style="background-color:white;color:black">
                    <h3>Microsoft ™ Windows 64bit</h3>
                    <p><a id="win64url">Yazılımı buradan indirin</a>, çalıştırın ve "Kur" veya "Bağlan" a basın.</p>
                ...
                <div id="wintab32" class="tabcontent" style="background-color:white;color:black">
                    ...
                    <p><a id="win32url">Yazılımı buradan indirin</a>, çalıştırın ve "Kur" veya "Bağlan" a basın.</p>
                    <div style="text-align:center">
                ...
                <div id="wintabarm64" class="tabcontent" style="background-color:white;color:black">
                    ...
                    <p><a id="winarm64url">Yazılımı buradan indirin</a>, çalıştırın ve "Kur" veya "Bağlan" a basın.</p>
                    <div style="text-align:center">
                ...
                <div id="linuxtab" class="tabcontent" style="background-color:white;color:black">
                    <h3>Linux</h3>
                    <p>Kurmak için aşağıdaki komutu bir kök terminalde kesip yapıştırın.</p>
                    <div id="linuxinstall" style="font-family:courier,'courier new',monospace;margin-left:30px"></div>
                    <input type="button" value="Panoya kopyala" style="margin-left:30px;margin-top:4px" onclick="copyToClipLinuxInstall()">
                    <p>Kaldırmak için aşağıdaki komutu kök olarak kesip yapıştırın.</p>
                    <div id="unlinuxinstall" style="font-family:courier,'courier new',monospace;margin-left:30px"></div>
                    <input type="button" value="Panoya kopyala" style="margin-left:30px;margin-top:4px" onclick="copyToClipLinuxUnInstall()">
                    <br><br>
                </div>
                ...
                <div id="macostab64" class="tabcontent" style="background-color:white;color:black">
                    ...
                    <p><a id="macos64url">Yükleyiciyi buradan indirin</a>üzerine sağ tıklayın veya "kontrol" e basın ve dosyaya tıklayın. Ardından "Aç" ı seçin ve talimatları izleyin.</p>
                    <div style="text-align:center">
                ...
                <div id="macostabarm64" class="tabcontent" style="background-color:white;color:black">
                    ...
                    <p><a id="macosarm64url">Yükleyiciyi buradan indirin</a>üzerine sağ tıklayın veya "kontrol" e basın ve dosyaya tıklayın. Ardından "Aç" ı seçin ve talimatları izleyin.</p>
                    <div style="text-align:center">
                ...
                <div id="macostab" class="tabcontent" style="background-color:white;color:black">
                    ...
                    <p><a id="macosurl">Yükleyiciyi buradan indirin</a>üzerine sağ tıklayın veya "kontrol" e basın ve dosyaya tıklayın. Ardından "Aç" ı seçin ve talimatları izleyin.</p>
                    <div style="text-align:center">
                ...
                <div id="androtab" class="tabcontent" style="background-color:white;color:black">
                    ...
                    <div style="min-height:220px">
                        <p>Ajan yazılımını Google Play Store veya APK dosyasını kullanarak indirin, kurun ve cihazınızı bu sunucuda kurmak için sağdaki QR kodunu veya aşağıdaki eşleştirme bağlantısını kullanın.</p>
                        <p>
                        ...
                        <p>
                            <a id="androidurl">Android APK İndir</a>
                        </p>
                        <p>
                            <a rel="noreferrer noopener" target="_blank" href="{{{magenturl}}}">Sunucu Eşleştirme Bağlantısı</a>
                        </p>
                    </div>
                </div>
                ...
                <div id="assistab" class="tabcontent" style="background-color:white;color:black">
                    ...
                    <div style="min-height:220px">
                        ...
                        <p>Asistan, sağ alt köşede yardım gerektiğinde kullanılabilecek bir Windows aracıdır. Bu, yönetilen bilgisayarda arka planda çalışan bir araç değil, sunucuya yalnızca istek yapıldığında bağlanılabilen bir araçtır.</p>
                        <p>
                            <a id="assisturl" rel="noreferrer noopener" target="_blank">Windows için Yardımcısı İndir</a>
                        </p>
                    </div>
                    <br style="clear:both">
                </div>
    ...
    <script>
        ...
        function startup() {
            ...
            if (groupName != '') {
                QH('groupname', format("{0} için Uzaktan Aracı Kurulumu", escapeHtml(groupName)));
                document.title = format("{0} - Agent Kurulumu", groupName);
            } else {
                document.title = "Agent Kurulumu";
            }
        ...
        function linuxBinarySetup() {
            var x = '<h3>' + "Linux / BSD / macOS İkili Yükleyici" + '</h3>', moreoptions = '';
            var opts = '<select id=aginsSelect onchange=addAgentToMeshClick() style=width:236px>';
            opts += '<option value=5 selected>' + "Linux / BSD / macOS İkili Yükleyici" + '</option>';
            opts += '</select>';
            ...
            x += addHtmlValue("Sistem tipi", '<select id=aginsSysType onchange=addAgentToMeshClick() style=width:236px>' + moreoptions + '</select>');
            x += '</div>';
            x += '<div id=aginsTypeDiv>';
            x += addHtmlValue("Kurulum Türü", '<select id=aginsType onchange=addAgentToMeshClick() style=width:236px><option value=0>' + "Arka plan ve etkileşimli" + '</option><option value=2>' + "Yalnızca arka plan" + '</option><option value=1>' + "Yalnızca etkileşimli" + '</option></select>');
            x += '</div><hr>';
            ...
            x += addHtmlValue("Mesh Ajan", '<a id=aginsbinlnk name="meshagents?id=' + meshid + '&installflags=0' + (urlargs.key?('&key=' + urlargs.key):'') + '">' + "ağ ajanı" + '</a> <img src=images/link4.png height=10 width=10 title="' + "Aracı URL'sini panoya kopyala" + '" style=cursor:pointer onclick=copyAgentUrl("meshagents?id=' + meshid + '&installflags=",1)>');
            x += addHtmlValue("Komut", '<input id=aginsbincmd type=text style="width:216px" readonly value=\'wget -O meshagent ' + ((serverNoProxy != 0) ?'--no-proxy':'') + ' --no-check-certificate \"https://' + servername + portStr + domainUrl + 'meshagents?id=' + meshid.split('$').join('%24').split('@').join('%40') + '\' /> <img src=images/link4.png height=10 width=10 title="' + "Aracı URL'sini panoya kopyala" + '" style=cursor:pointer onclick=copyAgentIdValue("aginsbincmd")>');
            x += '</div>';
            QH('linuxbinarytab', x);
            addAgentToMeshClick();
        }
