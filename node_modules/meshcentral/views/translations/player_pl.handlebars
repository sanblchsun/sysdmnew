        ...
        <div id="deskarea0">
            <div id="deskarea1" class="areaHead">
                <div class="toright2">
                    ...
                    <input id="ConvertAsWebM" style="display:none" type="button" value="Konwertuj na WebM" onclick="saveAsWebMfile()">&nbsp;
                </div>
                <div>
                    <input id="OpenFileButton" type="button" value="Otwórz Plik..." onclick="openfile()" style="display:none">
            ...
            <div id="deskarea4" class="areaFoot">
                ...
                <div>
                    &nbsp;
                    <input id="PlayButton" type="button" value="Odtwarzaj" disabled="disabled" onclick="play()">
                    <input id="PauseButton" type="button" value="Wstrzymaj" disabled="disabled" onclick="pause()">
                    <input id="RestartButton" type="button" value="Restart" disabled="disabled" onclick="restart()">
                    <select id="PlaySpeed" onchange="this.blur();">
                        <option value="4">1/4 Prędkości</option>
                        <option value="2">1/2 Prędkości</option>
                        <option value="1" selected="">Normalna Prędkość</option>
                        <option value="0.5">Prędkość 2x</option>
                        <option value="0.25">Prędkość 4x</option>
                        <option value="0.1">Prędkość 10x</option>
                    </select>
    ...
    <script>
        ...
        function start() {
            ...
            browser = (function (agent) {
                switch (true) {
                    case agent.indexOf("krawędź") > -1: return "Krawędź MS (EdgeHtml)";
                    case agent.indexOf("krawędź") > -1: return "MS Edge Chrom";
                    case agent.indexOf("opr") > -1 && !!window.opr: return "opera";
                    case agent.indexOf("chrom") > -1 && !!window.chrome: return "chrom";
                    case agent.indexOf("trójząb") > -1: return "Internet Explorer";
            ...
            if (urlargs.stream != null) {
                QV('metadatadiv', true);
                QH('metadatadiv', "Łączenie z serwerem...");
        ...
        function processFirstBlock(type, flags, time, data) {
            ...
            var x = '';
            x += addInfo("Czas", recFileMetadata.time);
            if (recFileEndTime != 0) { var secs = Math.floor((recFileEndTime - time) / 1000); x += addInfo("Czas trwania", format("{0} sekund{1}", secs, (secs > 1) ? 's' : '')); }
            x += addInfo("Nazwa użytkownika", recFileMetadata.username);
            x += addInfo("Identyfikator użytkownika", recFileMetadata.userid);
            x += addInfo("Identyfikator sesji", recFileMetadata.sessionid);
            if (recFileMetadata.ipaddr1 && recFileMetadata.ipaddr2) { x += addInfo("Adresy", format("{0} do {1}", recFileMetadata.ipaddr1, recFileMetadata.ipaddr2)); }
            if (recFileMetadata.devicename) { x += addInfo("Nazwa Urządzenia", recFileMetadata.devicename); }
            x += addInfo("Identyfikator węzła", recFileMetadata.nodeid);
            if (recFileMetadata.protocol) {
                var p = recFileMetadata.protocol;
                if (p == 1) { p = "Terminak MeshCentral"; }
                else if (p == 2) { p = "Pulpit MeshCentral"; }
                else if (p == 6) { p = "PowerShell jako Admin"; }
                else if (p == 8) { p = "Powłoka Użytkownika"; }
                else if (p == 9) { p = "Użyj PowerShell"; }
                else if (p == 100) { p = "Intel&reg; AMT WSMAN"; }
                else if (p == 101) { p = "Intel&reg; AMT Przekierowanie"; }
                else if ((p == 102) || (p == 200 && recFileMetadata.bpp != null)) { p = "Intel&reg; AMT KVM"; }
                else if (p == 200) { p = "MeshMessenger"; }
                x += addInfoNoEsc("Protokół", p);
            }
            var encQualityStr = "2 bajty na piksel";
            if (recFileMetadata.bpp == 1) { encQualityStr = "1 bajt na piksel" }
            if (recFileMetadata.graymode) { if (recFileMetadata.lowcolor) { encQualityStr += ", 16 odcieni szarości"; } else { encQualityStr += ", 256 odcieni szarości"; } }
            x += addInfoNoEsc("Jakość Kodowania", encQualityStr);

            if (recFileMetadata.indexInterval) {
                recFileIndexBasePtr = recFilePtr;
                x += addInfoNoEsc("Szukanie", format("Indeksowane co {0} sekund", recFileMetadata.indexInterval));
            ...
            if ((recFileMetadata.protocol == 1) || (recFileMetadata.protocol == 6) || (recFileMetadata.protocol == 8) || (recFileMetadata.protocol == 9)) {
                ...
                recFileProtocol = 1;
                x += '<br /><br /><span style=color:gray>' + "Naciśnij [spację], aby odtwarzać/wstrzymać." + '</span>';
                QE('PlayButton', true);
        ...
        function processBlockEx(type, flags, time, data, forced) {
            ...
            if ((type == 3) && (recFileProtocol == 102)) {
                var tile = new Image();
                tile.src = "dane:obraz/png;base64," + btoa(data);
        ...
        function cleanup() {
            ...
            QV('metadatadiv', true);
            if (urlargs.stream == null) {
                QH('metadatadiv', '<span style=\"font-family:Arial,Helvetica Neue,Helvetica,sans-serif;font-size:28px\">MeshCentral Session Player</span><br /><br /><span style=color:gray>' + "Przeciągnij i upuść plik .mcrec lub kliknij na \"Otwórz Plik...\"" + '</span>');
            } else {
                QH('metadatadiv', '');
            }
            QV('DeskParent', true);
        ...
        function saveAsWebMfile() {
            var x = '';
            x += addHtmlValue4("Prędkość odświeżania", '<select id=webmframerate style=width:200px><option value=100 selected>' + "10 klatek/s" + '</option><option value=50>' + "20 klatek/s" + '</option></select>');
            x += addHtmlValue4("Jakość", '<select id=webmquality style=width:200px><option value=90>' + "90%" + '</option><option value=80>' + "80%" + '</option><option value=60 selected>' + "60%" + '</option><option value=40>' + "40%" + '</option><option value=20>' + "20%" + '</option><option value=10>' + "10%" + '</option></select>');
            setDialogMode(2, "Konwertuj na WebM", 3, saveAsWebMfileEx, x);
        }
        ...
        function openfile() {
            ...
            setDialogMode(2, "Otwórz Plik...", 3, openfileEx, x);
