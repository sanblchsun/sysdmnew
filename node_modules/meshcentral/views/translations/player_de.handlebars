        ...
        <div id="deskarea0">
            <div id="deskarea1" class="areaHead">
                <div class="toright2">
                    ...
                    <input id="ConvertAsWebM" style="display:none" type="button" value="In WebM umwandeln" onclick="saveAsWebMfile()">&nbsp;
                </div>
                <div>
                    <input id="OpenFileButton" type="button" value="Datei öffnen..." onclick="openfile()" style="display:none">
            ...
            <div id="deskarea4" class="areaFoot">
                ...
                <div>
                    &nbsp;
                    <input id="PlayButton" type="button" value="Abspielen" disabled="disabled" onclick="play()">
                    <input id="PauseButton" type="button" value="Pause" disabled="disabled" onclick="pause()">
                    <input id="RestartButton" type="button" value="Neustart" disabled="disabled" onclick="restart()">
                    <select id="PlaySpeed" onchange="this.blur();">
                        <option value="4">1/4 Geschwindigkeit</option>
                        <option value="2">1/2 Geschwindigkeit</option>
                        <option value="1" selected="">Normale Geschwindigkeit</option>
                        <option value="0.5">2x Geschwindigkeit</option>
                        <option value="0.25">4x Geschwindigkeit</option>
                        <option value="0.1">10x Geschwindigkeit</option>
                    </select>
    ...
    <script>
        ...
        function start() {
            ...
            browser = (function (agent) {
                switch (true) {
                    case agent.indexOf("Kante") > -1: return "MS Edge (EdgeHtml)";
                    case agent.indexOf("kantig") > -1: return "MS Edge Chrom";
                    case agent.indexOf("opr") > -1 && !!window.opr: return "Opera";
                    case agent.indexOf("Chrome") > -1 && !!window.chrome: return "Chrome";
                    case agent.indexOf("Dreizack") > -1: return "Internet Explorer";
            ...
            if (urlargs.stream != null) {
                QV('metadatadiv', true);
                QH('metadatadiv', "Verbindung zum Server herstellen...");
        ...
        function processFirstBlock(type, flags, time, data) {
            ...
            var x = '';
            x += addInfo("Zeit", recFileMetadata.time);
            if (recFileEndTime != 0) { var secs = Math.floor((recFileEndTime - time) / 1000); x += addInfo("Dauer", format("{0} Sekunde{1}", secs, (secs > 1) ? 's' : '')); }
            x += addInfo("Benutzername", recFileMetadata.username);
            x += addInfo("Benutzer-ID", recFileMetadata.userid);
            x += addInfo("Sitzungs-ID", recFileMetadata.sessionid);
            if (recFileMetadata.ipaddr1 && recFileMetadata.ipaddr2) { x += addInfo("Adressen", format("{0} bis {1}", recFileMetadata.ipaddr1, recFileMetadata.ipaddr2)); }
            if (recFileMetadata.devicename) { x += addInfo("Gerätename", recFileMetadata.devicename); }
            ...
            if (recFileMetadata.protocol) {
                ...
                else if (p == 8) { p = "Benutzer-Shell"; }
                else if (p == 9) { p = "Benutzer-PowerShell"; }
                else if (p == 100) { p = "Intel&reg; AMT WSMAN"; }
                else if (p == 101) { p = "Intel&reg; AMT Umleitung"; }
                else if ((p == 102) || (p == 200 && recFileMetadata.bpp != null)) { p = "Intel&reg; AMT KVM"; }
                else if (p == 200) { p = "MeshMessenger"; }
                x += addInfoNoEsc("Protokoll", p);
            }
            var encQualityStr = "2 Byte pro Pixel";
            if (recFileMetadata.bpp == 1) { encQualityStr = "1 Byte pro Pixel" }
            if (recFileMetadata.graymode) { if (recFileMetadata.lowcolor) { encQualityStr += ", 16 Grautöne"; } else { encQualityStr += ", 256 Grautöne"; } }
            x += addInfoNoEsc("Kodierungsqualität", encQualityStr);

            if (recFileMetadata.indexInterval) {
                recFileIndexBasePtr = recFilePtr;
                x += addInfoNoEsc("Ich suche", format("Indiziert alle {0} Sekunden", recFileMetadata.indexInterval));
            ...
            if ((recFileMetadata.protocol == 1) || (recFileMetadata.protocol == 6) || (recFileMetadata.protocol == 8) || (recFileMetadata.protocol == 9)) {
                ...
                recFileProtocol = 1;
                x += '<br /><br /><span style=color:gray>' + "Für Abspielen/Pause [Leertaste] drücken," + '</span>';
                QE('PlayButton', true);
        ...
        function processBlockEx(type, flags, time, data, forced) {
            ...
            if ((type == 3) && (recFileProtocol == 102)) {
                var tile = new Image();
                tile.src = "Daten: image / png; base64," + btoa(data);
        ...
        function cleanup() {
            ...
            QV('metadatadiv', true);
            if (urlargs.stream == null) {
                QH('metadatadiv', '<span style=\"font-family:Arial,Helvetica Neue,Helvetica,sans-serif;font-size:28px\">MeshCentral Session Player</span><br /><br /><span style=color:gray>' + "Ziehen sie eine .mcrec-Datei hierher oder klicken Sie auf \"Datei öffnen...\"" + '</span>');
            } else {
                QH('metadatadiv', '');
            }
            QV('DeskParent', true);
        ...
        function saveAsWebMfile() {
            var x = '';
            x += addHtmlValue4("Bildrate", '<select id=webmframerate style=width:200px><option value=100 selected>' + "10 Bilder/sek" + '</option><option value=50>' + "20 Bilder/sek" + '</option></select>');
            x += addHtmlValue4("Qualität", '<select id=webmquality style=width:200px><option value=90>' + "90%" + '</option><option value=80>' + "80%" + '</option><option value=60 selected>' + "60%" + '</option><option value=40>' + "40%" + '</option><option value=20>' + "20%" + '</option><option value=10>' + "10%" + '</option></select>');
            setDialogMode(2, "In WebM umwandeln", 3, saveAsWebMfileEx, x);
        }
        ...
        function openfile() {
            ...
            setDialogMode(2, "Datei öffnen...", 3, openfileEx, x);
