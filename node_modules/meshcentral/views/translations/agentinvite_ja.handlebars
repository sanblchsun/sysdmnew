<!DOCTYPE html><html lang="ja"><head>
    ...
    <div id="container">
        ...
        <div id="column_l" style="overflow-y:auto">
            <h1><span id="groupname">リモートエージェントのインストール</span></h1>
            <p>
                リモートオペレーターがデスクトップやファイルなどのコンピューターに安全にアクセスできるようにするアプリケーションをインストールするよう招待されました。この招待が予期されていて、誰がコンピュータにアクセスするかがわかっている場合にのみ、以下の手順に従ってください。オペレーティングシステムを選択し、以下の手順に従ってインストールします。
            </p>
            <div>
                <div class="tab">
                    ...
                    <button id="tlinuxbinarytab" class="tablinks" onclick="openTab(event, 'linuxbinarytab')">Linux / BSD / macOSバイナリインストーラー</button>
                    <button id="tmacostab64" class="tablinks" onclick="openTab(event, 'macostab64')">macOS x86-64ビット</button>
                    <button id="tmacostabarm64" class="tablinks" onclick="openTab(event, 'macostabarm64')">macOS ARM (64ビット)</button>
                    ...
                    <button id="tassistab" class="tablinks" onclick="openTab(event, 'assistab')">アシスタント</button>
                </div>
                ...
                <div id="wintabarm64" class="tabcontent" style="background-color:white;color:black">
                    ...
                    <p><a id="winarm64url">ここからソフトウェアをダウンロードしてください</a>、実行して「インストール」または「接続」を押します。</p>
                    <div style="text-align:center">
                ...
                <div id="macostab64" class="tabcontent" style="background-color:white;color:black">
                    ...
                    <p><a id="macos64url">ここからインストーラーをダウンロードしてください</a>、それを右クリックするか、「control」を押してファイルをクリックします。次に、「開く」を選択し、指示に従います。</p>
                    <div style="text-align:center">
                ...
                <div id="macostabarm64" class="tabcontent" style="background-color:white;color:black">
                    ...
                    <p><a id="macosarm64url">ここからインストーラーをダウンロードしてください</a>、それを右クリックするか、「control」を押してファイルをクリックします。次に、「開く」を選択し、指示に従います。</p>
                    <div style="text-align:center">
                ...
                <div id="androtab" class="tabcontent" style="background-color:white;color:black">
                    ...
                    <div style="min-height:220px">
                        <p>Google PlayストアまたはAPKファイルを使用してエージェントソフトウェアをダウンロードし、インストールして、右側のQRコードまたは下のペアリングリンクを使用して、このサーバーにデバイスをセットアップします。</p>
                        <p>
                        ...
                        <p>
                            <a id="androidurl">AndroidAPKダウンロード</a>
                        </p>
                        <p>
                            <a rel="noreferrer noopener" target="_blank" href="{{{magenturl}}}">サーバーペアリングリンク</a>
                        </p>
                    </div>
                </div>
                ...
                <div id="assistab" class="tabcontent" style="background-color:white;color:black">
                    ...
                    <div style="min-height:220px">
                        ...
                        <p>アシスタントは、ヘルプが必要なときに使用できるWindowsシステムトレイツールです。これは、管理対象コンピューターのバックグラウンドで実行されるエージェントではなく、要求された場合にのみサーバーに接続するツールです。</p>
                        <p>
                            <a id="assisturl" rel="noreferrer noopener" target="_blank">Windows用のアシスタントをダウンロード</a>
                        </p>
                    </div>
                    <br style="clear:both">
                </div>
    ...
    <script>
        ...
        function startup() {
            ...
            if (groupName != '') {
                QH('groupname', format("{0}のリモートエージェントインストール", escapeHtml(groupName)));
                document.title = format("{0}-エージェントのインストール", groupName);
            } else {
                document.title = "エージェントのインストール";
            }
        ...
        function linuxBinarySetup() {
            var x = '<h3>' + "Linux / BSD / macOSバイナリインストーラー" + '</h3>', moreoptions = '';
            var opts = '<select id=aginsSelect onchange=addAgentToMeshClick() style=width:236px>';
            opts += '<option value=5 selected>' + "Linux / BSD / macOSバイナリインストーラー" + '</option>';
            opts += '</select>';
            ...
            x += addHtmlValue("システムタイプ", '<select id=aginsSysType onchange=addAgentToMeshClick() style=width:236px>' + moreoptions + '</select>');
            x += '</div>';
            ...
            x += addHtmlValue("メッシュエージェント", '<a id=aginsbinlnk name="meshagents?id=' + meshid + '&installflags=0' + (urlargs.key?('&key=' + urlargs.key):'') + '">' + "MeshAgent" + '</a> <img src=images/link4.png height=10 width=10 title="' + "エージェントのURLをクリップボードにコピー" + '" style=cursor:pointer onclick=copyAgentUrl("meshagents?id=' + meshid + '&installflags=",1)>');
            x += addHtmlValue("コマンド", '<input id=aginsbincmd type=text style="width:216px" readonly value=\'wget -O meshagent ' + ((serverNoProxy != 0) ?'--no-proxy':'') + ' --no-check-certificate \"https://' + servername + portStr + domainUrl + 'meshagents?id=' + meshid.split('$').join('%24').split('@').join('%40') + '\' /> <img src=images/link4.png height=10 width=10 title="' + "エージェントのURLをクリップボードにコピー" + '" style=cursor:pointer onclick=copyAgentIdValue("aginsbincmd")>');
            x += '</div>';
            QH('linuxbinarytab', x);
            addAgentToMeshClick();
        }
