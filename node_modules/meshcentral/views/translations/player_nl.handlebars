        ...
        <div id="deskarea0">
            <div id="deskarea1" class="areaHead">
                <div class="toright2">
                    ...
                    <input id="ConvertAsWebM" style="display:none" type="button" value="Converteer naar WebM" onclick="saveAsWebMfile()">&nbsp;
                </div>
                <div>
                    <input id="OpenFileButton" type="button" value="Open bestand..." onclick="openfile()" style="display:none">
    ...
    <script>
        ...
        function start() {
            ...
            if (urlargs.stream != null) {
                QV('metadatadiv', true);
                QH('metadatadiv', "Verbinden met de server...");
        ...
        function processFirstBlock(type, flags, time, data) {
            ...
            if (recFileMetadata.protocol) {
                ...
                else if (p == 6) { p = "Beheerder PowerShell"; }
                else if (p == 8) { p = "Gebruiker Shell"; }
                else if (p == 9) { p = "Gebruiker PowerShell"; }
                else if (p == 100) { p = "Intel&reg; AMT WSMAN"; }
                else if (p == 101) { p = "Intel&reg; AMT omleiding"; }
            ...
            if (recFileMetadata.graymode) { if (recFileMetadata.lowcolor) { encQualityStr += ", 16 grijstinten"; } else { encQualityStr += ", 256 grijstinten"; } }
            x += addInfoNoEsc("Coderingskwaliteit", encQualityStr);

            if (recFileMetadata.indexInterval) {
                recFileIndexBasePtr = recFilePtr;
                x += addInfoNoEsc("Op zoek naar", format("Geïndexeerd om de {0} seconden", recFileMetadata.indexInterval));
        ...
        function saveAsWebMfile() {
            var x = '';
            x += addHtmlValue4("Frameverhouding", '<select id=webmframerate style=width:200px><option value=100 selected>' + "10 frames/sec" + '</option><option value=50>' + "20 frames/sec" + '</option></select>');
            x += addHtmlValue4("Kwaliteit", '<select id=webmquality style=width:200px><option value=90>' + "90%" + '</option><option value=80>' + "80%" + '</option><option value=60 selected>' + "60%" + '</option><option value=40>' + "40%" + '</option><option value=20>' + "20%" + '</option><option value=10>' + "10%" + '</option></select>');
            setDialogMode(2, "Converteer naar WebM", 3, saveAsWebMfileEx, x);
        }
