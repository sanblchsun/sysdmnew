<!DOCTYPE html><html dir="ltr" xmlns="http://www.w3.org/1999/xhtml"><head lang="en">
        ...
        <div id="deskarea0" style="position:relative">
            <div id="deskarea1" class="areaHead">
                ...
                <div>
                    ...
                    <input id="DisconnectButton" type="button" value="Verbreken" onclick="connectButton()">
    ...
    <script>
        ...
        var StatusStrs = ["Verbroken", "Verbinden...", "Setup...", "Verbonden"];
        var state = 0;
        var socket = null;
        var user = '';
        var pass = '';
        ...
        function sshTunnelAuthDialog(j, func) {
            var x = '';
            if (j.askkeypass) {
                x += addHtmlValue("Authenticatie", '<select id=dp2authmethod style=width:230px onchange=sshAuthUpdate(event)><option value=3 selected>' + "Opgeslagen sleutel" + '</option><option value=1>' + "Gebruikersnaam wachtwoord" + '</option><option value=2>' + "Gebruikersnaam en sleutel" + '</option></select>');
            } else {
                x += addHtmlValue("Authenticatie", '<select id=dp2authmethod style=width:230px onchange=sshAuthUpdate(event)><option value=1 selected>' + "Gebruikersnaam wachtwoord" + '</option><option value=2>' + "Gebruikersnaam en sleutel" + '</option></select>');
            }
            x += '<div id=d2userauth style=display:none>';
            x += addHtmlValue("Gebruikersnaam", '<input id=dp2user style=width:230px maxlength=64 autocomplete=off onkeyup=sshAuthUpdate(event) />');
            x += '</div>';
            x += '<div id=d2passauth style=display:none>';
            x += addHtmlValue("Wachtwoord", '<input type=password id=dp2pass style=width:230px maxlength=64 autocomplete=off onkeyup=sshAuthUpdate(event) />');
            if ((features & 1) == 0) { x += addHtmlValue('', '<label><input id=dp2keep type=checkbox>' + "Onthoud inloggegevens" + '</label>'); }
            x += '</div><div id=d2keyauth style=display:none>';
            x += addHtmlValue("Sleutelbestand", '<input type=file id=dp2key style=width:230px maxlength=64 autocomplete=off onchange=sshAuthUpdate(event) />' + '<div id=d2badkey style=font-size:x-small>' + "Het sleutelbestand moet de OpenSSH-indeling hebben." + '</div>');
            x += addHtmlValue("Sleutelwachtwoord", '<input type=password id=dp2keypass style=width:230px maxlength=64 autocomplete=off onkeyup=sshAuthUpdate(event) />');
            if ((features & 1) == 0) {
                x += addHtmlValue('', '<label><input id=dp2keep1 type=checkbox onchange=sshAuthUpdate(event)>' + "Onthoud gebruikeruser & sleutel" + '</label>');
                x += addHtmlValue('', '<label><input id=dp2keep2 type=checkbox>' + "Onthoud wachtwoord" + '</label>');
            }
            x += '</div>';
            if (j.askkeypass) {
                x += '<div id=d2keyauth2 style=display:none>';
                x += addHtmlValue("Wachtwoord", '<input type=password id=dp2keypass2 style=width:230px maxlength=64 autocomplete=off onkeyup=sshAuthUpdate(event) />');
                x += '</div>';
            }
            ...
            socket.onmessage = function (data) {
                ...
                if (data.data[0] == '{') {
                    ...
                    switch (json.action) {
                        ...
                        case 'autherror': { setDialogMode(2, "Authenticatie", 1, null, "Kan niet verifiëren."); break; }
                        case 'sessionerror': { setDialogMode(2, "Sessie", 1, null, "Sessie verlopen."); break; }
                        case 'sessiontimeout': { setDialogMode(2, "Sessie", 1, null, "Sessie timeout."); break; }
                    }
                } else if (data.data[0] == '~') {
