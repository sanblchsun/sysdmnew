        ...
        <div id="deskarea0">
            <div id="deskarea1" class="areaHead">
                <div class="toright2">
                    ...
                    <input id="ConvertAsWebM" style="display:none" type="button" value="Конвертувати у WebM" onclick="saveAsWebMfile()">&nbsp;
                </div>
                <div>
                    <input id="OpenFileButton" type="button" value="Відкрити Файл..." onclick="openfile()" style="display:none">
            ...
            <div id="deskarea4" class="areaFoot">
                ...
                <div>
                    &nbsp;
                    <input id="PlayButton" type="button" value="Запустити" disabled="disabled" onclick="play()">
                    <input id="PauseButton" type="button" value="Пауза" disabled="disabled" onclick="pause()">
                    <input id="RestartButton" type="button" value="Перезапустити" disabled="disabled" onclick="restart()">
                    <select id="PlaySpeed" onchange="this.blur();">
                        <option value="4">1/4 Швидкість</option>
                        <option value="2">1/2 Швидкість</option>
                        <option value="1" selected="">Нормальна швидкість</option>
                        <option value="0.5">2x Швидкість</option>
                        <option value="0.25">4x Швидкість</option>
                        <option value="0.1">10x Швидкість</option>
                    </select>
    ...
    <script>
        ...
        function start() {
            ...
            browser = (function (agent) {
                switch (true) {
                    case agent.indexOf("кут") > -1: return "MS Edge (EdgeHtml)";
                    case agent.indexOf("край") > -1: return "MS Edge Chromium";
                    case agent.indexOf("opr") > -1 && !!window.opr: return "Опера";
                    case agent.indexOf("Chrome") > -1 && !!window.chrome: return "Chrome";
                    case agent.indexOf("тризуб") > -1: return "Internet Explorer";
                    case agent.indexOf("firefox") > -1: return "firefox";
                    case agent.indexOf("Сафарі") > -1: return "Сафарі";
                    default: return "інше";
                }
            ...
            if (urlargs.stream != null) {
                QV('metadatadiv', true);
                QH('metadatadiv', "Підключення до сервера...");
        ...
        function processFirstBlock(type, flags, time, data) {
            ...
            var x = '';
            x += addInfo("Період", recFileMetadata.time);
            if (recFileEndTime != 0) { var secs = Math.floor((recFileEndTime - time) / 1000); x += addInfo("Тривалість", format("{0} секунд{1}", secs, (secs > 1) ? 's' : '')); }
            x += addInfo("Ім'я користувача", recFileMetadata.username);
            x += addInfo("ID користувача", recFileMetadata.userid);
            x += addInfo("Ідентифікатор сеансу", recFileMetadata.sessionid);
            if (recFileMetadata.ipaddr1 && recFileMetadata.ipaddr2) { x += addInfo("Адреси", format("{0} зробити {1}", recFileMetadata.ipaddr1, recFileMetadata.ipaddr2)); }
            if (recFileMetadata.devicename) { x += addInfo("Назва Пристрою", recFileMetadata.devicename); }
            x += addInfo("NodeID", recFileMetadata.nodeid);
            if (recFileMetadata.protocol) {
                var p = recFileMetadata.protocol;
                if (p == 1) { p = "Термінал MeshCentral"; }
                else if (p == 2) { p = "Стільниця MeshCentral"; }
                else if (p == 6) { p = "PowerShell від імені адміністратора"; }
                else if (p == 8) { p = "Консоль користувача"; }
                else if (p == 9) { p = "PowerShell користувача"; }
                else if (p == 100) { p = "Intel&reg; AMT WSMAN"; }
                else if (p == 101) { p = "Перенаправлення Intel&reg; AMT"; }
                else if ((p == 102) || (p == 200 && recFileMetadata.bpp != null)) { p = "Intel&reg; AMT KVM"; }
                else if (p == 200) { p = "MeshMessenger"; }
                x += addInfoNoEsc("Протокол", p);
            }
            var encQualityStr = "2 байта на піксель";
            if (recFileMetadata.bpp == 1) { encQualityStr = "1 байт на піксель" }
            if (recFileMetadata.graymode) { if (recFileMetadata.lowcolor) { encQualityStr += ", 16 відтінків"; } else { encQualityStr += ", 256 відтінків"; } }
            x += addInfoNoEsc("Якість Кодування", encQualityStr);

            if (recFileMetadata.indexInterval) {
                recFileIndexBasePtr = recFilePtr;
                x += addInfoNoEsc("Шукаю", format("Індексується кожні {0} секунд(и)", recFileMetadata.indexInterval));
            ...
            if ((recFileMetadata.protocol == 1) || (recFileMetadata.protocol == 6) || (recFileMetadata.protocol == 8) || (recFileMetadata.protocol == 9)) {
                ...
                recFileProtocol = 1;
                x += '<br /><br /><span style=color:gray>' + "Натисніть [пробіл] для відтворення/паузи." + '</span>';
                QE('PlayButton', true);
        ...
        function processBlockEx(type, flags, time, data, forced) {
            ...
            if ((type == 3) && (recFileProtocol == 102)) {
                var tile = new Image();
                tile.src = "дані:зображення/png;base64," + btoa(data);
        ...
        function cleanup() {
            ...
            QV('metadatadiv', true);
            if (urlargs.stream == null) {
                QH('metadatadiv', '<span style=\"font-family:Arial,Helvetica Neue,Helvetica,sans-serif;font-size:28px\">MeshCentral Session Player</span><br /><br /><span style=color:gray>' + "Перетягніть файл .mcrec або клікніть \"Відкрити файл...\"" + '</span>');
            } else {
                QH('metadatadiv', '');
            }
            QV('DeskParent', true);
        ...
        function saveAsWebMfile() {
            var x = '';
            x += addHtmlValue4("Частота кадрів", '<select id=webmframerate style=width:200px><option value=100 selected>' + "10 кадрів за секунду" + '</option><option value=50>' + "20 кадрів за секунду" + '</option></select>');
            x += addHtmlValue4("Якість", '<select id=webmquality style=width:200px><option value=90>' + "90%" + '</option><option value=80>' + "80%" + '</option><option value=60 selected>' + "60%" + '</option><option value=40>' + "40%" + '</option><option value=20>' + "20%" + '</option><option value=10>' + "10%" + '</option></select>');
            setDialogMode(2, "Конвертувати у WebM", 3, saveAsWebMfileEx, x);
        }
        ...
        function openfile() {
            ...
            setDialogMode(2, "Відкрити Файл...", 3, openfileEx, x);
