<!DOCTYPE html><html dir="ltr" xmlns="http://www.w3.org/1999/xhtml"><head lang="en">
    ...
    <div id="p11" class="noselect" style="overflow:hidden">
        <div id="deskarea0" style="position:relative">
            <div id="deskarea1" class="areaHead">
                ...
                <div>
                    <input id="ConnectButton" style="display:none" type="button" value="Подключиться" onclick="connectButton()">
                    <input id="DisconnectButton" type="button" value="Разъединить" onclick="connectButton()">
    ...
    <script>
        ...
        var StatusStrs = ["Отключен", "Подключение...", "Установка...", "Подключено"];
        var state = 0;
        var socket = null;
        var user = '';
        var pass = '';
        ...
        function sshTunnelAuthDialog(j, func) {
            var x = '';
            if (j.askkeypass) {
                x += addHtmlValue("Аутентификация", '<select id=dp2authmethod style=width:230px onchange=sshAuthUpdate(event)><option value=3 selected>' + "Сохраненный ключ" + '</option><option value=1>' + "Имя пользователя и пароль" + '</option><option value=2>' + "Имя пользователя и ключ" + '</option></select>');
            } else {
                x += addHtmlValue("Аутентификация", '<select id=dp2authmethod style=width:230px onchange=sshAuthUpdate(event)><option value=1 selected>' + "Имя пользователя и пароль" + '</option><option value=2>' + "Имя пользователя и ключ" + '</option></select>');
            }
            x += '<div id=d2userauth style=display:none>';
            x += addHtmlValue("Имя пользователя", '<input id=dp2user style=width:230px maxlength=64 autocomplete=off onkeyup=sshAuthUpdate(event) />');
            x += '</div>';
            x += '<div id=d2passauth style=display:none>';
            x += addHtmlValue("Пароль", '<input type=password id=dp2pass style=width:230px maxlength=64 autocomplete=off onkeyup=sshAuthUpdate(event) />');
            if ((features & 1) == 0) { x += addHtmlValue('', '<label><input id=dp2keep type=checkbox>' + "Запомнить учетные данные" + '</label>'); }
            x += '</div><div id=d2keyauth style=display:none>';
            x += addHtmlValue("Ключевой файл", '<input type=file id=dp2key style=width:230px maxlength=64 autocomplete=off onchange=sshAuthUpdate(event) />' + '<div id=d2badkey style=font-size:x-small>' + "Ключевой файл должен быть в формате OpenSSH." + '</div>');
            x += addHtmlValue("Пароль ключа", '<input type=password id=dp2keypass style=width:230px maxlength=64 autocomplete=off onkeyup=sshAuthUpdate(event) />');
            if ((features & 1) == 0) {
                x += addHtmlValue('', '<label><input id=dp2keep1 type=checkbox onchange=sshAuthUpdate(event)>' + "Запомнить пользователя и ключ" + '</label>');
                x += addHtmlValue('', '<label><input id=dp2keep2 type=checkbox>' + "Запомнить пароль" + '</label>');
            }
            x += '</div>';
            if (j.askkeypass) {
                x += '<div id=d2keyauth2 style=display:none>';
                x += addHtmlValue("Пароль", '<input type=password id=dp2keypass2 style=width:230px maxlength=64 autocomplete=off onkeyup=sshAuthUpdate(event) />');
                x += '</div>';
            }
            ...
            socket.onmessage = function (data) {
                ...
                if (data.data[0] == '{') {
                    ...
                    switch (json.action) {
                        ...
                        case 'autherror': { setDialogMode(2, "Аутентификация", 1, null, "Невозможно аутентифицировать."); break; }
                        case 'sessionerror': { setDialogMode(2, "Сессия", 1, null, "Сессия истекла."); break; }
                        case 'sessiontimeout': { setDialogMode(2, "Сессия", 1, null, "Тайм-аут сеанса."); break; }
                    }
                } else if (data.data[0] == '~') {
