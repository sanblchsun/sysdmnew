        ...
        <div id="deskarea0">
            <div id="deskarea1" class="areaHead">
                <div class="toright2">
                    ...
                    <input id="ConvertAsWebM" style="display:none" type="button" value="Конвертировать в WebM" onclick="saveAsWebMfile()">&nbsp;
                </div>
                <div>
                    <input id="OpenFileButton" type="button" value="Открыть файл..." onclick="openfile()" style="display:none">
            ...
            <div id="deskarea4" class="areaFoot">
                ...
                <div>
                    &nbsp;
                    <input id="PlayButton" type="button" value="Воспроизвести" disabled="disabled" onclick="play()">
                    <input id="PauseButton" type="button" value="Пауза" disabled="disabled" onclick="pause()">
                    <input id="RestartButton" type="button" value="Перезапуск" disabled="disabled" onclick="restart()">
                    <select id="PlaySpeed" onchange="this.blur();">
                        <option value="4">1/4 Скорость</option>
                        <option value="2">1/2 Скорость</option>
                        <option value="1" selected="">Нормальная скорость</option>
                        <option value="0.5">2x Скорость</option>
                        <option value="0.25">4x Скорость</option>
                        <option value="0.1">10x Скорость</option>
                    </select>
    ...
    <script>
        ...
        function start() {
            ...
            if (urlargs.stream != null) {
                QV('metadatadiv', true);
                QH('metadatadiv', "Подключение к серверу...");
        ...
        function processFirstBlock(type, flags, time, data) {
            ...
            var x = '';
            x += addInfo("Время", recFileMetadata.time);
            if (recFileEndTime != 0) { var secs = Math.floor((recFileEndTime - time) / 1000); x += addInfo("Длительность", format("{0} секунд{1}", secs, (secs > 1) ? 's' : '')); }
            x += addInfo("Имя пользователя", recFileMetadata.username);
            x += addInfo("UserID", recFileMetadata.userid);
            x += addInfo("ID сессии", recFileMetadata.sessionid);
            if (recFileMetadata.ipaddr1 && recFileMetadata.ipaddr2) { x += addInfo("Адреса", format("{0} до {1}", recFileMetadata.ipaddr1, recFileMetadata.ipaddr2)); }
            if (recFileMetadata.devicename) { x += addInfo("Имя устройства", recFileMetadata.devicename); }
            x += addInfo("NodeID", recFileMetadata.nodeid);
            if (recFileMetadata.protocol) {
                var p = recFileMetadata.protocol;
                if (p == 1) { p = "Терминал MeshCentral"; }
                else if (p == 2) { p = "Рабочий стол MeshCentral"; }
                else if (p == 6) { p = "PowerShell от Админа"; }
                else if (p == 8) { p = "Консоль от Пользователя"; }
                else if (p == 9) { p = "PowerShell от Пользователя"; }
                else if (p == 100) { p = "Intel&reg; AMT WSMAN"; }
                else if (p == 101) { p = "Перенаправление Intel&reg; AMT"; }
                else if ((p == 102) || (p == 200 && recFileMetadata.bpp != null)) { p = "Intel&reg; AMT KVM"; }
                else if (p == 200) { p = "MeshMessenger"; }
                x += addInfoNoEsc("Протокол", p);
            }
            var encQualityStr = "2 байта на пиксель";
            if (recFileMetadata.bpp == 1) { encQualityStr = "1 байт на пиксель" }
            if (recFileMetadata.graymode) { if (recFileMetadata.lowcolor) { encQualityStr += ", 16 оттенков серого"; } else { encQualityStr += ", 256 оттенков серого"; } }
            x += addInfoNoEsc("Качество кодирования", encQualityStr);

            if (recFileMetadata.indexInterval) {
                recFileIndexBasePtr = recFilePtr;
                x += addInfoNoEsc("Ищу", format("Индексируется каждые {0} секунд", recFileMetadata.indexInterval));
            ...
            if ((recFileMetadata.protocol == 1) || (recFileMetadata.protocol == 6) || (recFileMetadata.protocol == 8) || (recFileMetadata.protocol == 9)) {
                ...
                recFileProtocol = 1;
                x += '<br /><br /><span style=color:gray>' + "Нажмите [пробел] для проигрывания/паузы." + '</span>';
                QE('PlayButton', true);
        ...
        function cleanup() {
            ...
            QV('metadatadiv', true);
            if (urlargs.stream == null) {
                QH('metadatadiv', '<span style=\"font-family:Arial,Helvetica Neue,Helvetica,sans-serif;font-size:28px\">MeshCentral Session Player</span><br /><br /><span style=color:gray>' + "Перетащите .mcrec файл или нажмите \"Открыть файл ... \"" + '</span>');
            } else {
                QH('metadatadiv', '');
            }
            QV('DeskParent', true);
        ...
        function saveAsWebMfile() {
            var x = '';
            x += addHtmlValue4("Частота кадров", '<select id=webmframerate style=width:200px><option value=100 selected>' + "10 кадров / сек" + '</option><option value=50>' + "20 кадров / сек" + '</option></select>');
            x += addHtmlValue4("Качество", '<select id=webmquality style=width:200px><option value=90>' + "90%" + '</option><option value=80>' + "80%" + '</option><option value=60 selected>' + "60%" + '</option><option value=40>' + "40%" + '</option><option value=20>' + "20%" + '</option><option value=10>' + "10%" + '</option></select>');
            setDialogMode(2, "Конвертировать в WebM", 3, saveAsWebMfileEx, x);
        }
        ...
        function openfile() {
            ...
            setDialogMode(2, "Открыть файл...", 3, openfileEx, x);
