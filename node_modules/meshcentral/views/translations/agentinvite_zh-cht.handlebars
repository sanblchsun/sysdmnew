<!DOCTYPE html><html lang="zh-cht"><head>
    ...
    <div id="container">
        ...
        <div id="column_l" style="overflow-y:auto">
            <h1><span id="groupname">遠程代理安裝</span></h1>
            <p>
                你已被邀請安裝一個應用程序，該應用程序將允許遠程操作員安全地訪問你的電腦，包括桌面和檔案。如果收到此邀請，並且只有知道誰將訪問你的電腦，請僅按照以下說明進行操作。選擇你的操作系統，然後按照以下說明進行安裝。
            </p>
            <div>
                <div class="tab">
                    <button id="twintab64" class="tablinks" onclick="openTab(event, 'wintab64')">Windows 64位</button>
                    <button id="twintab32" class="tablinks" onclick="openTab(event, 'wintab32')">Windows 32位</button>
                    ...
                    <button id="tlinuxbinarytab" class="tablinks" onclick="openTab(event, 'linuxbinarytab')">Linux / BSD / macOS二進制安裝軟體</button>
                    <button id="tmacostab64" class="tablinks" onclick="openTab(event, 'macostab64')">macOS x86-64位</button>
                    <button id="tmacostabarm64" class="tablinks" onclick="openTab(event, 'macostabarm64')">macOS ARM (64位)</button>
                    ...
                    <button id="tassistab" class="tablinks" onclick="openTab(event, 'assistab')">助手</button>
                </div>

                <div id="wintab64" class="tabcontent" style="background-color:white;color:black">
                    <h3>Microsoft™Windows 64位</h3>
                    <p><a id="win64url">在此處下載軟體</a>，運行它，然後按“安裝”或“連接”。</p>
                ...
                <div id="wintab32" class="tabcontent" style="background-color:white;color:black">
                    <h3>Microsoft™Windows 32位</h3>
                    <p><a id="win32url">在此處下載軟體</a>，運行它，然後按“安裝”或“連接”。</p>
                    <div style="text-align:center">
                ...
                <div id="wintabarm64" class="tabcontent" style="background-color:white;color:black">
                    ...
                    <p><a id="winarm64url">在此處下載軟體</a>，運行它，然後按“安裝”或“連接”。</p>
                    <div style="text-align:center">
                ...
                <div id="linuxtab" class="tabcontent" style="background-color:white;color:black">
                    <h3>Linux</h3>
                    <p>要安装，剪貼以下命令到root終端。</p>
                    <div id="linuxinstall" style="font-family:courier,'courier new',monospace;margin-left:30px"></div>
                    <input type="button" value="複製到剪貼板" style="margin-left:30px;margin-top:4px" onclick="copyToClipLinuxInstall()">
                    <p>要卸載，請以root用戶身份剪貼以下命令。</p>
                    <div id="unlinuxinstall" style="font-family:courier,'courier new',monospace;margin-left:30px"></div>
                    <input type="button" value="複製到剪貼板" style="margin-left:30px;margin-top:4px" onclick="copyToClipLinuxUnInstall()">
                    <br><br>
                </div>
                ...
                <div id="macostab64" class="tabcontent" style="background-color:white;color:black">
                    ...
                    <p><a id="macos64url">在此處下載安裝程序</a>，右鍵單擊它或按“控制”，然後單擊檔案。然後選擇“打開”並按照說明操作。</p>
                    <div style="text-align:center">
                ...
                <div id="macostabarm64" class="tabcontent" style="background-color:white;color:black">
                    ...
                    <p><a id="macosarm64url">在此處下載安裝程序</a>，右鍵單擊它或按“控制”，然後單擊檔案。然後選擇“打開”並按照說明操作。</p>
                    <div style="text-align:center">
                ...
                <div id="macostab" class="tabcontent" style="background-color:white;color:black">
                    ...
                    <p><a id="macosurl">在此處下載安裝程序</a>，右鍵單擊它或按“控制”，然後單擊檔案。然後選擇“打開”並按照說明操作。</p>
                    <div style="text-align:center">
                ...
                <div id="androtab" class="tabcontent" style="background-color:white;color:black">
                    ...
                    <div style="min-height:220px">
                        <p>使用 Google Play 商店或 APK 文件下載代理軟件，安裝它並使用右側的二維碼或下面的配對鏈接在此服務器上設置您的設備。</p>
                        <p>
                        ...
                        <p>
                            <a id="androidurl">安卓版APK下載</a>
                        </p>
                        <p>
                            <a rel="noreferrer noopener" target="_blank" href="{{{magenturl}}}">服務器配對鏈接</a>
                        </p>
                    </div>
                </div>
                ...
                <div id="assistab" class="tabcontent" style="background-color:white;color:black">
                    ...
                    <div style="min-height:220px">
                        ...
                        <p>Assistant 是一個 Windows 系統托盤工具，可以在需要幫助時使用。這不是在託管計算機的後台運行的代理，而是一個僅在請求時連接到服務器的工具。</p>
                        <p>
                            <a id="assisturl" rel="noreferrer noopener" target="_blank">Windows 下載助手</a>
                        </p>
                    </div>
                    <br style="clear:both">
                </div>
    ...
    <script>
        ...
        function startup() {
            ...
            if (groupName != '') {
                QH('groupname', format("{0}的遠程代理安裝", escapeHtml(groupName)));
                document.title = format("{0}-代理安裝", groupName);
            } else {
                document.title = "代理安裝";
            }
        ...
        function linuxBinarySetup() {
            var x = '<h3>' + "Linux / BSD / macOS二進制安裝軟體" + '</h3>', moreoptions = '';
            var opts = '<select id=aginsSelect onchange=addAgentToMeshClick() style=width:236px>';
            opts += '<option value=5 selected>' + "Linux / BSD / macOS二進制安裝軟體" + '</option>';
            opts += '</select>';
            ...
            x += addHtmlValue("系統類型", '<select id=aginsSysType onchange=addAgentToMeshClick() style=width:236px>' + moreoptions + '</select>');
            x += '</div>';
            x += '<div id=aginsTypeDiv>';
            x += addHtmlValue("安裝方式", '<select id=aginsType onchange=addAgentToMeshClick() style=width:236px><option value=0>' + "背景與互動" + '</option><option value=2>' + "僅背景" + '</option><option value=1>' + "僅限互動" + '</option></select>');
            x += '</div><hr>';
            ...
            x += addHtmlValue("Mesh Agent", '<a id=aginsbinlnk name="meshagents?id=' + meshid + '&installflags=0' + (urlargs.key?('&key=' + urlargs.key):'') + '">' + "MeshAgent" + '</a> <img src=images/link4.png height=10 width=10 title="' + "將代理URL複製到剪貼板" + '" style=cursor:pointer onclick=copyAgentUrl("meshagents?id=' + meshid + '&installflags=",1)>');
            x += addHtmlValue("命令", '<input id=aginsbincmd type=text style="width:216px" readonly value=\'wget -O meshagent ' + ((serverNoProxy != 0) ?'--no-proxy':'') + ' --no-check-certificate \"https://' + servername + portStr + domainUrl + 'meshagents?id=' + meshid.split('$').join('%24').split('@').join('%40') + '\' /> <img src=images/link4.png height=10 width=10 title="' + "將代理URL複製到剪貼板" + '" style=cursor:pointer onclick=copyAgentIdValue("aginsbincmd")>');
            x += '</div>';
            QH('linuxbinarytab', x);
            addAgentToMeshClick();
        }
