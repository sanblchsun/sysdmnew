        ...
        <div id="deskarea0">
            <div id="deskarea1" class="areaHead">
                <div class="toright2">
                    ...
                    <input id="ConvertAsWebM" style="display:none" type="button" value="WebM'ye Dönüştür" onclick="saveAsWebMfile()">&nbsp;
                </div>
                <div>
                    <input id="OpenFileButton" type="button" value="Açık dosya..." onclick="openfile()" style="display:none">
            ...
            <div id="deskarea4" class="areaFoot">
                ...
                <div>
                    &nbsp;
                    <input id="PlayButton" type="button" value="Oyna" disabled="disabled" onclick="play()">
                    <input id="PauseButton" type="button" value="Duraklat" disabled="disabled" onclick="pause()">
                    <input id="RestartButton" type="button" value="Tekrar başlat" disabled="disabled" onclick="restart()">
                    <select id="PlaySpeed" onchange="this.blur();">
                        <option value="4">1/4 Hız</option>
                        <option value="2">1/2 Hız</option>
                        <option value="1" selected="">Normal hız</option>
                        <option value="0.5">2x Hız</option>
                        <option value="0.25">4x Hız</option>
                        <option value="0.1">10x Hız</option>
                    </select>
    ...
    <script>
        ...
        function start() {
            ...
            browser = (function (agent) {
                switch (true) {
                    case agent.indexOf("köşe") > -1: return "MS Edge (EdgeHtml)";
                    case agent.indexOf("kenar") > -1: return "MS Kenar Kromu";
                    case agent.indexOf("operasyon") > -1 && !!window.opr: return "opera";
                    case agent.indexOf("Chrome") > -1 && !!window.chrome: return "Chrome";
                    case agent.indexOf("trident") > -1: return "Internet Explorer";
                    case agent.indexOf("firefox") > -1: return "firefox";
                    case agent.indexOf("safari") > -1: return "safari";
                    default: return "başka";
                }
            ...
            if (urlargs.stream != null) {
                QV('metadatadiv', true);
                QH('metadatadiv', "Sunucuya baglanıyor...");
        ...
        function processFirstBlock(type, flags, time, data) {
            ...
            var x = '';
            x += addInfo("Zaman", recFileMetadata.time);
            if (recFileEndTime != 0) { var secs = Math.floor((recFileEndTime - time) / 1000); x += addInfo("Süresi", format("{0} saniye {1}", secs, (secs > 1) ? 's' : '')); }
            x += addInfo("Kullanıcı adı", recFileMetadata.username);
            x += addInfo("Kullanıcı kimliği", recFileMetadata.userid);
            x += addInfo("Oturum kimliği", recFileMetadata.sessionid);
            if (recFileMetadata.ipaddr1 && recFileMetadata.ipaddr2) { x += addInfo("Adres", format("{0} - {1}", recFileMetadata.ipaddr1, recFileMetadata.ipaddr2)); }
            if (recFileMetadata.devicename) { x += addInfo("Cihaz adı", recFileMetadata.devicename); }
            x += addInfo("Cihaz Kimliği", recFileMetadata.nodeid);
            if (recFileMetadata.protocol) {
                var p = recFileMetadata.protocol;
                if (p == 1) { p = "MeshCentral Terminali"; }
                else if (p == 2) { p = "MeshCentral Masaüstü"; }
                else if (p == 6) { p = "Yönetici PowerShell"; }
                else if (p == 8) { p = "Kullanıcı Kabuğu"; }
                else if (p == 9) { p = "Kullanıcı PowerShell"; }
                else if (p == 100) { p = "Intel&reg; AMT WSMAN"; }
                else if (p == 101) { p = "Intel&reg; AMT Yeniden Yönlendirme"; }
                else if ((p == 102) || (p == 200 && recFileMetadata.bpp != null)) { p = "Intel&reg; AMT KVM"; }
                else if (p == 200) { p = "MeshMessenger"; }
                x += addInfoNoEsc("Protokol", p);
            }
            var encQualityStr = "piksel başına 2 bayt";
            if (recFileMetadata.bpp == 1) { encQualityStr = "piksel başına 1 bayt" }
            if (recFileMetadata.graymode) { if (recFileMetadata.lowcolor) { encQualityStr += ", 16 gri"; } else { encQualityStr += ", 256 gri"; } }
            x += addInfoNoEsc("Kodlama Kalitesi", encQualityStr);

            if (recFileMetadata.indexInterval) {
                recFileIndexBasePtr = recFilePtr;
                x += addInfoNoEsc("Aranıyor", format("Her {0} saniyede bir dizine eklenir", recFileMetadata.indexInterval));
            ...
            if ((recFileMetadata.protocol == 1) || (recFileMetadata.protocol == 6) || (recFileMetadata.protocol == 8) || (recFileMetadata.protocol == 9)) {
                ...
                recFileProtocol = 1;
                x += '<br /><br /><span style=color:gray>' + "Oynatmak / duraklatmak için [boşluk] tuşuna basın." + '</span>';
                QE('PlayButton', true);
        ...
        function processBlockEx(type, flags, time, data, forced) {
            ...
            if ((type == 3) && (recFileProtocol == 102)) {
                var tile = new Image();
                tile.src = "veri: resim / png; base64," + btoa(data);
        ...
        function cleanup() {
            ...
            QV('metadatadiv', true);
            if (urlargs.stream == null) {
                QH('metadatadiv', '<span style=\"font-family:Arial,Helvetica Neue,Helvetica,sans-serif;font-size:28px\">MeshCentral Session Player</span><br /><br /><span style=color:gray>' + "Bir .mcrec dosyasını sürükleyip bırakın veya \"Dosya Aç ...\" ı tıklayın" + '</span>');
            } else {
                QH('metadatadiv', '');
            }
            QV('DeskParent', true);
        ...
        function saveAsWebMfile() {
            var x = '';
            x += addHtmlValue4("Kare hızı", '<select id=webmframerate style=width:200px><option value=100 selected>' + "10 kare/sn" + '</option><option value=50>' + "20 kare/sn" + '</option></select>');
            x += addHtmlValue4("Kalite", '<select id=webmquality style=width:200px><option value=90>' + "90%" + '</option><option value=80>' + "80%" + '</option><option value=60 selected>' + "60%" + '</option><option value=40>' + "40%" + '</option><option value=20>' + "20%" + '</option><option value=10>' + "10%" + '</option></select>');
            setDialogMode(2, "WebM'ye Dönüştür", 3, saveAsWebMfileEx, x);
        }
        ...
        function openfile() {
            ...
            setDialogMode(2, "Açık dosya...", 3, openfileEx, x);
