<!DOCTYPE html><html dir="ltr" xmlns="http://www.w3.org/1999/xhtml"><head lang="en">
    ...
    <div id="p11" class="noselect" style="overflow:hidden">
        <div id="deskarea0" style="position:relative">
            <div id="deskarea1" class="areaHead">
                ...
                <div>
                    <input id="ConnectButton" style="display:none" type="button" value="Yhdistä" onclick="connectButton()">
                    <input id="DisconnectButton" type="button" value="Katkaise yhteys" onclick="connectButton()">
    ...
    <script>
        ...
        var StatusStrs = ["Yhteys katkaistu", "Yhdistetään...", "Asennus...", "Yhdistetty"];
        var state = 0;
        var socket = null;
        var user = '';
        var pass = '';
        ...
        function sshTunnelAuthDialog(j, func) {
            var x = '';
            if (j.askkeypass) {
                x += addHtmlValue("Todentaminen", '<select id=dp2authmethod style=width:230px onchange=sshAuthUpdate(event)><option value=3 selected>' + "Tallennettu avain" + '</option><option value=1>' + "Käyttäjänimi Salasana" + '</option><option value=2>' + "Käyttäjätunnus ja avain" + '</option></select>');
            } else {
                x += addHtmlValue("Todentaminen", '<select id=dp2authmethod style=width:230px onchange=sshAuthUpdate(event)><option value=1 selected>' + "Käyttäjänimi Salasana" + '</option><option value=2>' + "Käyttäjätunnus ja avain" + '</option></select>');
            }
            x += '<div id=d2userauth style=display:none>';
            x += addHtmlValue("Käyttäjätunnus", '<input id=dp2user style=width:230px maxlength=64 autocomplete=off onkeyup=sshAuthUpdate(event) />');
            x += '</div>';
            x += '<div id=d2passauth style=display:none>';
            x += addHtmlValue("Salasana", '<input type=password id=dp2pass style=width:230px maxlength=64 autocomplete=off onkeyup=sshAuthUpdate(event) />');
            if ((features & 1) == 0) { x += addHtmlValue('', '<label><input id=dp2keep type=checkbox>' + "Muista valtakirjat" + '</label>'); }
            x += '</div><div id=d2keyauth style=display:none>';
            x += addHtmlValue("Avaintiedosto", '<input type=file id=dp2key style=width:230px maxlength=64 autocomplete=off onchange=sshAuthUpdate(event) />' + '<div id=d2badkey style=font-size:x-small>' + "Avaintiedoston on oltava OpenSSH -muodossa." + '</div>');
            x += addHtmlValue("Avaimen salasana", '<input type=password id=dp2keypass style=width:230px maxlength=64 autocomplete=off onkeyup=sshAuthUpdate(event) />');
            if ((features & 1) == 0) {
                x += addHtmlValue('', '<label><input id=dp2keep1 type=checkbox onchange=sshAuthUpdate(event)>' + "Muista käyttäjä ja avain" + '</label>');
                x += addHtmlValue('', '<label><input id=dp2keep2 type=checkbox>' + "Muista salasana" + '</label>');
            }
            x += '</div>';
            if (j.askkeypass) {
                x += '<div id=d2keyauth2 style=display:none>';
                x += addHtmlValue("Salasana", '<input type=password id=dp2keypass2 style=width:230px maxlength=64 autocomplete=off onkeyup=sshAuthUpdate(event) />');
                x += '</div>';
            }
            ...
            socket.onmessage = function (data) {
                ...
                if (data.data[0] == '{') {
                    ...
                    switch (json.action) {
                        ...
                        case 'autherror': { setDialogMode(2, "Todentaminen", 1, null, "Todentaminen ei onnistu."); break; }
                        case 'sessionerror': { setDialogMode(2, "Istunto", 1, null, "Istunto päättyi."); break; }
                        case 'sessiontimeout': { setDialogMode(2, "Istunto", 1, null, "Istunto vanhentunut."); break; }
                    }
                } else if (data.data[0] == '~') {
