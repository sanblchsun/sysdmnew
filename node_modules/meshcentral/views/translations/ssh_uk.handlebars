<!DOCTYPE html><html dir="ltr" xmlns="http://www.w3.org/1999/xhtml"><head lang="en">
    ...
    <div id="p11" class="noselect" style="overflow:hidden">
        <div id="deskarea0" style="position:relative">
            <div id="deskarea1" class="areaHead">
                ...
                <div>
                    <input id="ConnectButton" style="display:none" type="button" value="Підключитись" onclick="connectButton()">
                    <input id="DisconnectButton" type="button" value="Відключити" onclick="connectButton()">
    ...
    <script>
        ...
        var StatusStrs = ["Відключено", "Підключення...", "Установити...", "Підключено"];
        var state = 0;
        var socket = null;
        var user = '';
        var pass = '';
        ...
        function sshTunnelAuthDialog(j, func) {
            var x = '';
            if (j.askkeypass) {
                x += addHtmlValue("Автентифікація", '<select id=dp2authmethod style=width:230px onchange=sshAuthUpdate(event)><option value=3 selected>' + "Збережений ключ" + '</option><option value=1>' + "Ім'я користувача та пароль" + '</option><option value=2>' + "Ім'я користувача та ключ" + '</option></select>');
            } else {
                x += addHtmlValue("Автентифікація", '<select id=dp2authmethod style=width:230px onchange=sshAuthUpdate(event)><option value=1 selected>' + "Ім'я користувача та пароль" + '</option><option value=2>' + "Ім'я користувача та ключ" + '</option></select>');
            }
            x += '<div id=d2userauth style=display:none>';
            x += addHtmlValue("Ім'я користувача", '<input id=dp2user style=width:230px maxlength=64 autocomplete=off onkeyup=sshAuthUpdate(event) />');
            x += '</div>';
            ...
            if ((features & 1) == 0) { x += addHtmlValue('', '<label><input id=dp2keep type=checkbox>' + "Запам'ятати облікові дані" + '</label>'); }
            x += '</div><div id=d2keyauth style=display:none>';
            x += addHtmlValue("Файл Ключа", '<input type=file id=dp2key style=width:230px maxlength=64 autocomplete=off onchange=sshAuthUpdate(event) />' + '<div id=d2badkey style=font-size:x-small>' + "Файл ключа мусить бути у форматі OpenSSH." + '</div>');
            ...
            if ((features & 1) == 0) {
                x += addHtmlValue('', '<label><input id=dp2keep1 type=checkbox onchange=sshAuthUpdate(event)>' + "Запам'ятати користувача та ключ" + '</label>');
                x += addHtmlValue('', '<label><input id=dp2keep2 type=checkbox>' + "Запам'ятати пароль" + '</label>');
            }
            x += '</div>';
            if (j.askkeypass) {
            ...
            socket.onmessage = function (data) {
                ...
                if (data.data[0] == '{') {
                    ...
                    switch (json.action) {
                        ...
                        case 'autherror': { setDialogMode(2, "Автентифікація", 1, null, "Не вдалося автентифікувати."); break; }
                        case 'sessionerror': { setDialogMode(2, "Сеанс", 1, null, "Сеанс закінчився."); break; }
                        case 'sessiontimeout': { setDialogMode(2, "Сеанс", 1, null, "Тайм-аут сеансу."); break; }
                    }
                } else if (data.data[0] == '~') {
