        ...
        <div id="deskarea0">
            <div id="deskarea1" class="areaHead">
                <div class="toright2">
                    ...
                    <input id="ConvertAsWebM" style="display:none" type="button" value="Converteix a WebM" onclick="saveAsWebMfile()">&nbsp;
                </div>
                <div>
                    <input id="OpenFileButton" type="button" value="Arxiu obert..." onclick="openfile()" style="display:none">
            ...
            <div id="deskarea4" class="areaFoot">
                ...
                <div>
                    &nbsp;
                    <input id="PlayButton" type="button" value="Jugar" disabled="disabled" onclick="play()">
                    <input id="PauseButton" type="button" value="Pausa" disabled="disabled" onclick="pause()">
                    <input id="RestartButton" type="button" value="Reinicia" disabled="disabled" onclick="restart()">
                    <select id="PlaySpeed" onchange="this.blur();">
                        <option value="4">1/4 de velocitat</option>
                        <option value="2">1/2 velocitat</option>
                        <option value="1" selected="">Velocitat normal</option>
                        <option value="0.5">2x velocitat</option>
                        <option value="0.25">Velocitat 4x</option>
                        <option value="0.1">Velocitat 10x</option>
                    </select>
    ...
    <script>
        ...
        function start() {
            ...
            browser = (function (agent) {
                switch (true) {
                    case agent.indexOf("vora") > -1: return "MS Edge (EdgeHtml)";
                    case agent.indexOf("edg") > -1: return "MS Edge Chromium";
                    case agent.indexOf("opr") > -1 && !!window.opr: return "òpera";
                    case agent.indexOf("cromat") > -1 && !!window.chrome: return "cromat";
            ...
            if (urlargs.stream != null) {
                QV('metadatadiv', true);
                QH('metadatadiv', "Connectant al servidor...");
        ...
        function processFirstBlock(type, flags, time, data) {
            ...
            if (recFileEndTime != 0) { var secs = Math.floor((recFileEndTime - time) / 1000); x += addInfo("Durada", format("{0} segon{1}", secs, (secs > 1) ? 's' : '')); }
            x += addInfo("Nom d'usuari", recFileMetadata.username);
            x += addInfo("ID d'usuari", recFileMetadata.userid);
            x += addInfo("ID de sessió", recFileMetadata.sessionid);
            if (recFileMetadata.ipaddr1 && recFileMetadata.ipaddr2) { x += addInfo("Adreces", format("{0} a {1}", recFileMetadata.ipaddr1, recFileMetadata.ipaddr2)); }
            if (recFileMetadata.devicename) { x += addInfo("Nom del dispositiu", recFileMetadata.devicename); }
            x += addInfo("NodeID", recFileMetadata.nodeid);
            if (recFileMetadata.protocol) {
                var p = recFileMetadata.protocol;
                if (p == 1) { p = "Terminal MeshCentral"; }
                else if (p == 2) { p = "Escriptori MeshCentral"; }
                else if (p == 6) { p = "Administrador PowerShell"; }
                else if (p == 8) { p = "Shell d'usuari"; }
                else if (p == 9) { p = "Usuari PowerShell"; }
                else if (p == 100) { p = "Intel&reg; AMT WSMAN"; }
                else if (p == 101) { p = "Redirecció Intel&reg; AMT"; }
                else if ((p == 102) || (p == 200 && recFileMetadata.bpp != null)) { p = "Intel&reg; AMT KVM"; }
                else if (p == 200) { p = "MeshMessenger"; }
                x += addInfoNoEsc("Protocol", p);
            }
            var encQualityStr = "2 bytes per píxel";
            if (recFileMetadata.bpp == 1) { encQualityStr = "1 byte per píxel" }
            if (recFileMetadata.graymode) { if (recFileMetadata.lowcolor) { encQualityStr += ", 16 grisos"; } else { encQualityStr += ", 256 grisos"; } }
            x += addInfoNoEsc("Qualitat de codificació", encQualityStr);

            if (recFileMetadata.indexInterval) {
                recFileIndexBasePtr = recFilePtr;
                x += addInfoNoEsc("Buscant", format("S'indexa cada {0} segons", recFileMetadata.indexInterval));
            ...
            if ((recFileMetadata.protocol == 1) || (recFileMetadata.protocol == 6) || (recFileMetadata.protocol == 8) || (recFileMetadata.protocol == 9)) {
                ...
                recFileProtocol = 1;
                x += '<br /><br /><span style=color:gray>' + "Premeu [espai] per reproduir/pausar." + '</span>';
                QE('PlayButton', true);
        ...
        function processBlockEx(type, flags, time, data, forced) {
            ...
            if ((type == 3) && (recFileProtocol == 102)) {
                var tile = new Image();
                tile.src = "dades:imatge/png;base64," + btoa(data);
        ...
        function cleanup() {
            ...
            QV('metadatadiv', true);
            if (urlargs.stream == null) {
                QH('metadatadiv', '<span style=\"font-family:Arial,Helvetica Neue,Helvetica,sans-serif;font-size:28px\">MeshCentral Session Player</span><br /><br /><span style=color:gray>' + "Arrossegueu i deixeu anar un fitxer .mcrec o feu clic a \"Obre el fitxer...\"" + '</span>');
            } else {
                QH('metadatadiv', '');
            }
            QV('DeskParent', true);
        ...
        function saveAsWebMfile() {
            var x = '';
            x += addHtmlValue4("Velocitat de fotogrames", '<select id=webmframerate style=width:200px><option value=100 selected>' + "10 fotogrames/s" + '</option><option value=50>' + "20 fotogrames/s" + '</option></select>');
            x += addHtmlValue4("Qualitat", '<select id=webmquality style=width:200px><option value=90>' + "90%" + '</option><option value=80>' + "80%" + '</option><option value=60 selected>' + "60%" + '</option><option value=40>' + "40%" + '</option><option value=20>' + "20%" + '</option><option value=10>' + "10%" + '</option></select>');
            setDialogMode(2, "Converteix a WebM", 3, saveAsWebMfileEx, x);
        }
        ...
        function openfile() {
            ...
            setDialogMode(2, "Arxiu obert...", 3, openfileEx, x);
