<!DOCTYPE html><html lang="nl" dir="ltr" xmlns="http://www.w3.org/1999/xhtml"><head>
    ...
    <div id="LeftSideToolBar" style="display:none;position:absolute;left:0;bottom:0;width:52px;top:0;background:#113962;background:linear-gradient(to bottom, #104893 0%,#113962 100%);color:white;border-right: 5px solid #BBB;">
        <div id="LeftMenuDesktop" class="slbbutton slbbuttonsel2" title="Bureaublad" onclick="go(11)">
        ...
        <div id="LeftMenuFiles" class="slbbutton" title="Bestanden" onclick="go(13)">
        ...
        <div id="deskarea0">
            <div id="deskarea1" class="areaHead" style="line-height:24px">
                ...
                <div>
                    ...
                    <span id="connectbutton1hspan">&nbsp;<input type="button" id="connectbutton1h" value="HW verbinden" title="Maak verbinding met behulp van Intel AMT hardware KVM" onclick="connectDesktop(event,2)" onkeypress="return false" onkeydown="return false" disabled="disabled"></span>
                    ...
                    &nbsp;<span id="deskstatus">Verbroken</span><span id="deskmetadata"></span>
                </div>
            </div>
            ...
            <div id="deskarea4" class="areaFoot" style="min-height:24px">
                <div class="toright2">
                    <span id="DeskLatency" style="line-height:22px;width:50px" title="Desktop sessie wachttijd"></span>
                    <span id="DeskTimer" style="line-height:22px" title="Sessie tijd"></span>&nbsp;
                    ...
                    <span id="DeskSaveImageButton" title="Bewaar een screenshot van het externe bureaublad"><img src="images/icon-camera.png" onclick="deskSaveImage()" height="16" width="16" style="padding-top:2px"></span>
                </div>
    ...
    <div id="p12" class="noselect" style="overflow:hidden;position:absolute;left:54px;top:0;right:0;bottom:0;display:none;background-color:black">
        ...
        <div class="areaFoot" style="position:absolute;bottom:0;left:0;right:0;height:24px">
            <div class="toright2">
                <span id="TermLatency" title="Latentie van terminalsessie"></span>
            ...
            <div>
                ...
                <input type="button" onkeypress="return false" onkeydown="return false" class="bottombutton" id="pastebutton" value="Plakken" title="Plak tekst in de terminal" onclick="showTermPasteDialog()" style="display:none">
            </div>
        </div>
        ...
        <div id="p13toolbar" style="position:absolute;left:0;top:0;right:0;bottom:28px">
            ...
            <div id="fileArea2" class="areaHead2" valign="bottom">
                ...
                <div>
                    ...
                    <input type="button" id="p13CopyButton" disabled="disabled" value="Kopiëren" onclick="p13copyFile(0)">&nbsp;
                    ...
                    <input type="button" id="p13RefreshButton" disabled="disabled" value="Verversen" onclick="p13folderup(9999)">&nbsp;
                    <input type="button" id="p13FindButton" disabled="disabled" value="Zoeken" onclick="p13findfile()">&nbsp;
                </div>
            </div>
    ...
    <div id="dialog" class="noselect" style="display:none">
        ...
        <div id="dialogBody">
            ...
            <div id="dialog4" style="">
                <input id="d4WrapButton" type="button" value="Wrap Aan" onclick="d4ToggleWrap()">
                ...
                <div id="d7meshkvm">
                    ...
                    <div id="d7desktopOtherSettings">
                        ...
                        <div id="d7otherset2" style="display:block">
                            <label style="display:block"><input type="checkbox" id="d7deskSwapMouse">Wissel muisknoppen</label>
                            <label style="display:block"><input type="checkbox" id="d7deskRemoteKeyMap">Gebruik de externe toetsenbord instelling</label>
                        </div>
                    </div>
                </div>
                <div id="d7amtkvm">
    ...
    <script>
        ...
        function updateMetadata(conn, elementid) {
            ...
            if (conn && (conn.State == 3)) {
                ...
                if (viewerCount > 1) { str = '<span onclick=showSessionMetadata(1) style=cursor:pointer>' + format(", {0} kijken", viewerCount) + '</span>'; }
            }
            QH('deskmetadata', str);
        ...
        function showSessionMetadata(cid) {
            ...
            if (conn && conn.metadata) {
                var x = '';
                if (conn.metadata.startTime) { x += addHtmlValue4("Start tijd", printDateTime(new Date(conn.metadata.startTime))); }
                if (conn.metadata.users) {
                    for (var i in conn.metadata.users) {
                        var val = (conn.metadata.users[i] == 1) ? "1 verbinding" : format("{0} verbindingen", conn.metadata.users[i]);
                        var username = i.split('/')[2];
                        if ((users != null) && (users[i] != null)) { username = users[i].name; }
                        x += addHtmlValue4(format("Gebuiker \"{0}\"", username), val);
                    }
                }
                setDialogMode(2, "Sessie informatie", 1, null, x, 'sessionMetadata' + cid);
            }
        }
        ...
        */
        ...
        function deskSaveImage() {
            if (xxdialogMode || desktop == null || desktop.State != 3) return;
            var d = new Date(), n = "Bureaublad" + '-' + d.getFullYear() + '-' + ('0' + (d.getMonth() + 1)).slice(-2) + '-' + ('0' + d.getDate()).slice(-2) + '-' + ('0' + d.getHours()).slice(-2) + '-' + ('0' + d.getMinutes()).slice(-2);
        ...
        function p13gotFiles(data) {
            ...
            if (data.action == 'findfile') {
                if (xxdialogTag == data.reqid) {
                    if (data.r == null) {
                        ...
                        xxdialogTag = null;
                        if (Q('d2findResults').innerHTML == '') { QH('d2findResults', '<div style=text-align:center;margin:10px><i>' + "Geen bestanden gevonden" + '</i></div>'); }
                    } else {
            ...
            if (data.action == 'dialogmessage') {
                if ((data.msg == null) && (xxdialogTag == 'fileMsgDialog')) {
                    ...
                    setDialogMode(2, "Bestandsbewerking", 10, p13fileOperationDialogEx, '<div style=margin:10px>' + "Bestanden comprimeren..." + '<div>', 'fileMsgDialog');
                } else if ((data.msg == 'zippingFile') && ((!xxdialogMode) || (xxdialogTag == 'fileMsgDialog'))) {
                    // Show the dialog box message
                    setDialogMode(2, "Bestandsbewerking", 10, p13fileOperationDialogEx, '<div style=margin:10px>' + EscapeHtml(data.file) + '<div><br /><progress value=' + EscapeHtml(data.progress) + ' style=width:100% max=100 />', 'fileMsgDialog');
                }
                return;
            }
        ...
        function p13findfile() {
            ...
            x += '<div style=margin-top:8px><input id=filefind_dlgCancelButton type=button value=' + "Sluiten" + ' style=float:right;width:80px;margin-left:5px onclick=p13findfileCancel()>';
            x += '<input id=filefind_dlgOkButton type=submit value=' + "Zoeken" + ' style=float:right;width:80px onclick=p13findfileEx()><br /><br /></div>';
            setDialogMode(2, "Zoek bestanden", 0, null, x);
            p13findfileValidate();
            Q('d2findFilter').focus();
        }

        function p13findfileDown(e) { if (e.code == "invoeren") { p13findfileEx(); } }
        ...
        function p13zipFiles() {
            ...
            setDialogMode(2, "Zip bestandsnaam", 3, p13zipFilesEx, '<input type=text id=p13renameinput maxlength=64 onkeyup=p13fileNameCheck(event) style=width:100% value="" />', { action: 'zip', path: p13filetreelocation.join('/'), files: inputFiles });
        ...
        function p13pasteFile() {
            ...
            if ((p13clipboard != null) && (p13clipboard.length > 0)) {
                if (p13clipboardCut == 0) {
                    if (p13clipboard.length > 1) { x = format("Bevestig kopiëren van {0} bestanden naar deze locatie?", p13clipboard.length); } else { x = format("Bevestig kopiëren van 1 bestand naar deze locatie?"); }
                } else {
                    if (p13clipboard.length > 1) { x = format("Verplaatsing van {0} bestanden naar deze locatie bevestigen?", p13clipboard.length); } else { x = format("Verplaatsing van 1 bestand naar deze locatie bevestigen?"); }
                }
            }
        ...
        function p13downloadfile(x, y, z, tag) {
            ...
            setDialogMode(2, "Download bestand", 10, p13downloadFileCancel, '<div>' + gdownloadFile.file + '</div><br /><progress id=d2progressBar style=width:100% value=0 max=' + z + ' />');
        }
        ...
        function d4ToggleWrap(update) {
            ...
            Q('d4WrapButton').value = ["Wrap: AAN", "Wrap: UIT"][d4EditWrapVal];
        ...
        function d4ToggleSize(update) {
            ...
            Q('d4SizeButton').value = ["Grootte: 150%", "Grootte: 150%", "Grootte: 150%", "Grootte: 200%"][d4EditSizeVal];
        ...
        function d4ToggleEncoding(update) {
            ...
            Q('d4EncodingButton').value = ["Codering: RAW","Codering: UTF8"][d4EditEncodingVal];
        ...
        function printFlexDateTime(d) { if (printDate(new Date()) == printDate(d)) { return format("Vervald om {0}", printTime(d)); } else { return format("vervalt op {0}", printDateTime(d)); } }
