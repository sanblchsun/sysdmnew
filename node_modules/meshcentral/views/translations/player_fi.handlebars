        ...
        <div id="deskarea0">
            <div id="deskarea1" class="areaHead">
                <div class="toright2">
                    ...
                    <input id="ConvertAsWebM" style="display:none" type="button" value="Muunna WebM: ksi" onclick="saveAsWebMfile()">&nbsp;
                </div>
                <div>
                    <input id="OpenFileButton" type="button" value="Avaa tiedosto..." onclick="openfile()" style="display:none">
            ...
            <div id="deskarea4" class="areaFoot">
                ...
                <div>
                    &nbsp;
                    <input id="PlayButton" type="button" value="Toista" disabled="disabled" onclick="play()">
                    <input id="PauseButton" type="button" value="Tauko" disabled="disabled" onclick="pause()">
                    <input id="RestartButton" type="button" value="Käynnistä uudelleen" disabled="disabled" onclick="restart()">
                    <select id="PlaySpeed" onchange="this.blur();">
                        <option value="4">1/4 Nopeus</option>
                        <option value="2">1/2 Nopeus</option>
                        <option value="1" selected="">Normaali nopeus</option>
                        <option value="0.5">2x Nopeus</option>
                        <option value="0.25">4x Nopeus</option>
                        <option value="0.1">10x Nopeus</option>
                    </select>
    ...
    <script>
        ...
        function start() {
            ...
            browser = (function (agent) {
                switch (true) {
                    case agent.indexOf("reuna") > -1: return "MS Edge (EdgeHtml)";
                    case agent.indexOf("edg") > -1: return "MS Edge Chromium";
                    case agent.indexOf("opr") > -1 && !!window.opr: return "ooppera";
                    case agent.indexOf("Chrome") > -1 && !!window.chrome: return "Chrome";
                    case agent.indexOf("kolmikärkinen") > -1: return "Internet Explorer";
            ...
            if (urlargs.stream != null) {
                QV('metadatadiv', true);
                QH('metadatadiv', "Yhdistetään palvelimeen...");
        ...
        function processFirstBlock(type, flags, time, data) {
            ...
            var x = '';
            x += addInfo("Aika", recFileMetadata.time);
            if (recFileEndTime != 0) { var secs = Math.floor((recFileEndTime - time) / 1000); x += addInfo("Kesto", format("{0} sekuntti {1}", secs, (secs > 1) ? 's' : '')); }
            x += addInfo("Käyttäjätunnus", recFileMetadata.username);
            x += addInfo("Käyttäjätunnus", recFileMetadata.userid);
            x += addInfo("Istunnon tunniste", recFileMetadata.sessionid);
            if (recFileMetadata.ipaddr1 && recFileMetadata.ipaddr2) { x += addInfo("Osoitteet", format("{0} - {1}", recFileMetadata.ipaddr1, recFileMetadata.ipaddr2)); }
            if (recFileMetadata.devicename) { x += addInfo("Laitteen Nimi", recFileMetadata.devicename); }
            x += addInfo("NodeID", recFileMetadata.nodeid);
            if (recFileMetadata.protocol) {
                var p = recFileMetadata.protocol;
                if (p == 1) { p = "MeshCentral-terminaali"; }
                else if (p == 2) { p = "MeshCentral Työpöytä"; }
                else if (p == 6) { p = "Järjestelmänvalvojan PowerShell"; }
                else if (p == 8) { p = "Käyttäjän kuori"; }
                else if (p == 9) { p = "Käyttäjä PowerShell"; }
                else if (p == 100) { p = "Intel&reg; AMT WSMAN"; }
                else if (p == 101) { p = "Intel&reg; AMT Uudelleenohjaus"; }
                else if ((p == 102) || (p == 200 && recFileMetadata.bpp != null)) { p = "Intel&reg; AMT KVM"; }
                else if (p == 200) { p = "MeshMessenger"; }
                x += addInfoNoEsc("Protokolla", p);
            }
            var encQualityStr = "2 tavua pikseliä kohden";
            if (recFileMetadata.bpp == 1) { encQualityStr = "1 tavu pikseliä kohden" }
            if (recFileMetadata.graymode) { if (recFileMetadata.lowcolor) { encQualityStr += ", 16 harmaata"; } else { encQualityStr += ", 256 harmaata"; } }
            x += addInfoNoEsc("Koodauksen laatu", encQualityStr);

            if (recFileMetadata.indexInterval) {
                recFileIndexBasePtr = recFilePtr;
                x += addInfoNoEsc("Haetaan", format("Hakemisto indeksoidaan {0} sekunnin välein", recFileMetadata.indexInterval));
            ...
            if ((recFileMetadata.protocol == 1) || (recFileMetadata.protocol == 6) || (recFileMetadata.protocol == 8) || (recFileMetadata.protocol == 9)) {
                ...
                recFileProtocol = 1;
                x += '<br /><br /><span style=color:gray>' + "Toista/keskeytä painamalla [välilyönti]." + '</span>';
                QE('PlayButton', true);
        ...
        function cleanup() {
            ...
            QV('metadatadiv', true);
            if (urlargs.stream == null) {
                QH('metadatadiv', '<span style=\"font-family:Arial,Helvetica Neue,Helvetica,sans-serif;font-size:28px\">MeshCentral Session Player</span><br /><br /><span style=color:gray>' + "Vedä ja pudota .mcrec-tiedosto tai napsauta \"Avaa tiedosto ...\"" + '</span>');
            } else {
                QH('metadatadiv', '');
            }
            QV('DeskParent', true);
        ...
        function saveAsWebMfile() {
            var x = '';
            x += addHtmlValue4("Ruudunpäivitysnopeus", '<select id=webmframerate style=width:200px><option value=100 selected>' + "10 kuvaa/s" + '</option><option value=50>' + "20 kuvaa/s" + '</option></select>');
            x += addHtmlValue4("Laatu", '<select id=webmquality style=width:200px><option value=90>' + "90%" + '</option><option value=80>' + "80%" + '</option><option value=60 selected>' + "60%" + '</option><option value=40>' + "40%" + '</option><option value=20>' + "20%" + '</option><option value=10>' + "10%" + '</option></select>');
            setDialogMode(2, "Muunna WebM: ksi", 3, saveAsWebMfileEx, x);
        }
        ...
        function openfile() {
            ...
            setDialogMode(2, "Avaa tiedosto...", 3, openfileEx, x);
