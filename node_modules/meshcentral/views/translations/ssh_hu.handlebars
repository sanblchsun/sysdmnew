<!DOCTYPE html><html dir="ltr" xmlns="http://www.w3.org/1999/xhtml"><head lang="en">
    ...
    <div id="p11" class="noselect" style="overflow:hidden">
        <div id="deskarea0" style="position:relative">
            <div id="deskarea1" class="areaHead">
                ...
                <div>
                    <input id="ConnectButton" style="display:none" type="button" value="Kapcsolódás" onclick="connectButton()">
                    <input id="DisconnectButton" type="button" value="Lekapcsolódás" onclick="connectButton()">
    ...
    <script>
        ...
        var StatusStrs = ["Lecsatlakozott", "Csatlakozás...", "Beállít...", "Csatlakozott"];
        var state = 0;
        var socket = null;
        var user = '';
        var pass = '';
        ...
        function sshTunnelAuthDialog(j, func) {
            var x = '';
            if (j.askkeypass) {
                x += addHtmlValue("Hitelesítés", '<select id=dp2authmethod style=width:230px onchange=sshAuthUpdate(event)><option value=3 selected>' + "Tárolt kulcs" + '</option><option value=1>' + "Felhasználónév és Jelszó" + '</option><option value=2>' + "Felhasználó néc és kulcs" + '</option></select>');
            } else {
                x += addHtmlValue("Hitelesítés", '<select id=dp2authmethod style=width:230px onchange=sshAuthUpdate(event)><option value=1 selected>' + "Felhasználónév és Jelszó" + '</option><option value=2>' + "Felhasználó néc és kulcs" + '</option></select>');
            }
            x += '<div id=d2userauth style=display:none>';
            x += addHtmlValue("Felhasználó név", '<input id=dp2user style=width:230px maxlength=64 autocomplete=off onkeyup=sshAuthUpdate(event) />');
            x += '</div>';
            x += '<div id=d2passauth style=display:none>';
            x += addHtmlValue("Jelszó", '<input type=password id=dp2pass style=width:230px maxlength=64 autocomplete=off onkeyup=sshAuthUpdate(event) />');
            if ((features & 1) == 0) { x += addHtmlValue('', '<label><input id=dp2keep type=checkbox>' + "Hitelesítő adatok megjegyzése" + '</label>'); }
            x += '</div><div id=d2keyauth style=display:none>';
            x += addHtmlValue("Kulcs fájl", '<input type=file id=dp2key style=width:230px maxlength=64 autocomplete=off onchange=sshAuthUpdate(event) />' + '<div id=d2badkey style=font-size:x-small>' + "A kulcsfájlnak OpenSSH formátumúnak kell lennie." + '</div>');
            x += addHtmlValue("Kulcs jelszó", '<input type=password id=dp2keypass style=width:230px maxlength=64 autocomplete=off onkeyup=sshAuthUpdate(event) />');
            if ((features & 1) == 0) {
                x += addHtmlValue('', '<label><input id=dp2keep1 type=checkbox onchange=sshAuthUpdate(event)>' + "Emlékezzen a felhaszálóra és kulcsra" + '</label>');
                x += addHtmlValue('', '<label><input id=dp2keep2 type=checkbox>' + "Elmékezzej a jelszóra" + '</label>');
            }
            x += '</div>';
            if (j.askkeypass) {
                x += '<div id=d2keyauth2 style=display:none>';
                x += addHtmlValue("Jelszó", '<input type=password id=dp2keypass2 style=width:230px maxlength=64 autocomplete=off onkeyup=sshAuthUpdate(event) />');
                x += '</div>';
            }
            ...
            socket.onmessage = function (data) {
                ...
                if (data.data[0] == '{') {
                    ...
                    switch (json.action) {
                        ...
                        case 'autherror': { setDialogMode(2, "Hitelesítés", 1, null, "Nem sikerült hitelesíteni."); break; }
                        case 'sessionerror': { setDialogMode(2, "Munkamenet", 1, null, "Munkamenet lejárt."); break; }
                        case 'sessiontimeout': { setDialogMode(2, "Munkamenet", 1, null, "Munkamenet időtúllépés."); break; }
                    }
                } else if (data.data[0] == '~') {
