<!DOCTYPE html><html dir="ltr" xmlns="http://www.w3.org/1999/xhtml"><head lang="en">
    ...
    <div id="p11" class="noselect" style="overflow:hidden">
        <div id="deskarea0" style="position:relative">
            <div id="deskarea1" class="areaHead">
                ...
                <div>
                    <input id="ConnectButton" style="display:none" type="button" value="連接" onclick="connectButton()">
                    <input id="DisconnectButton" type="button" value="斷線" onclick="connectButton()">
    ...
    <script>
        ...
        var StatusStrs = ["已斷線", "正在連線...", "設定...", "已連接"];
        var state = 0;
        var socket = null;
        var user = '';
        var pass = '';
        ...
        function sshTunnelAuthDialog(j, func) {
            var x = '';
            if (j.askkeypass) {
                x += addHtmlValue("驗證", '<select id=dp2authmethod style=width:230px onchange=sshAuthUpdate(event)><option value=3 selected>' + "存儲密鑰" + '</option><option value=1>' + "用戶名密碼" + '</option><option value=2>' + "用戶名和密鑰" + '</option></select>');
            } else {
                x += addHtmlValue("驗證", '<select id=dp2authmethod style=width:230px onchange=sshAuthUpdate(event)><option value=1 selected>' + "用戶名密碼" + '</option><option value=2>' + "用戶名和密鑰" + '</option></select>');
            }
            x += '<div id=d2userauth style=display:none>';
            x += addHtmlValue("用戶名", '<input id=dp2user style=width:230px maxlength=64 autocomplete=off onkeyup=sshAuthUpdate(event) />');
            x += '</div>';
            x += '<div id=d2passauth style=display:none>';
            x += addHtmlValue("密碼", '<input type=password id=dp2pass style=width:230px maxlength=64 autocomplete=off onkeyup=sshAuthUpdate(event) />');
            if ((features & 1) == 0) { x += addHtmlValue('', '<label><input id=dp2keep type=checkbox>' + "記住憑據" + '</label>'); }
            x += '</div><div id=d2keyauth style=display:none>';
            x += addHtmlValue("密鑰文件", '<input type=file id=dp2key style=width:230px maxlength=64 autocomplete=off onchange=sshAuthUpdate(event) />' + '<div id=d2badkey style=font-size:x-small>' + "密鑰文件必須是 OpenSSH 格式。" + '</div>');
            x += addHtmlValue("密鑰密碼", '<input type=password id=dp2keypass style=width:230px maxlength=64 autocomplete=off onkeyup=sshAuthUpdate(event) />');
            if ((features & 1) == 0) {
                x += addHtmlValue('', '<label><input id=dp2keep1 type=checkbox onchange=sshAuthUpdate(event)>' + "記住用戶和密鑰" + '</label>');
                x += addHtmlValue('', '<label><input id=dp2keep2 type=checkbox>' + "記住密碼" + '</label>');
            }
            x += '</div>';
            if (j.askkeypass) {
                x += '<div id=d2keyauth2 style=display:none>';
                x += addHtmlValue("密碼", '<input type=password id=dp2keypass2 style=width:230px maxlength=64 autocomplete=off onkeyup=sshAuthUpdate(event) />');
                x += '</div>';
            }
            ...
            socket.onmessage = function (data) {
                ...
                if (data.data[0] == '{') {
                    ...
                    switch (json.action) {
                        ...
                        case 'autherror': { setDialogMode(2, "驗證", 1, null, "無法進行身份驗證。"); break; }
                        case 'sessionerror': { setDialogMode(2, "節", 1, null, "會話已過期。"); break; }
                        case 'sessiontimeout': { setDialogMode(2, "節", 1, null, "會話超時。"); break; }
                    }
                } else if (data.data[0] == '~') {
