<!DOCTYPE html><html dir="ltr" xmlns="http://www.w3.org/1999/xhtml"><head lang="en">
    ...
    <div id="p11" class="noselect" style="overflow:hidden">
        <div id="deskarea0" style="position:relative">
            <div id="deskarea1" class="areaHead">
                ...
                <div>
                    <input id="ConnectButton" style="display:none" type="button" value="जुडिये" onclick="connectButton()">
                    <input id="DisconnectButton" type="button" value="डिस्कनेक्ट" onclick="connectButton()">
    ...
    <script>
        ...
        var StatusStrs = ["डिस्कनेक्ट किया गया", "कनेक्ट ...", "सेट अप...", "जुड़े हुए"];
        var state = 0;
        var socket = null;
        var user = '';
        var pass = '';
        ...
        function sshTunnelAuthDialog(j, func) {
            var x = '';
            if (j.askkeypass) {
                x += addHtmlValue("प्रमाणीकरण", '<select id=dp2authmethod style=width:230px onchange=sshAuthUpdate(event)><option value=3 selected>' + "संग्रहीत कुंजी" + '</option><option value=1>' + "प्रयोक्ता नाम पासवर्ड" + '</option><option value=2>' + "उपयोगकर्ता नाम और कुंजी" + '</option></select>');
            } else {
                x += addHtmlValue("प्रमाणीकरण", '<select id=dp2authmethod style=width:230px onchange=sshAuthUpdate(event)><option value=1 selected>' + "प्रयोक्ता नाम पासवर्ड" + '</option><option value=2>' + "उपयोगकर्ता नाम और कुंजी" + '</option></select>');
            }
            x += '<div id=d2userauth style=display:none>';
            x += addHtmlValue("उपयोगकर्ता नाम", '<input id=dp2user style=width:230px maxlength=64 autocomplete=off onkeyup=sshAuthUpdate(event) />');
            x += '</div>';
            x += '<div id=d2passauth style=display:none>';
            x += addHtmlValue("कुंजिका", '<input type=password id=dp2pass style=width:230px maxlength=64 autocomplete=off onkeyup=sshAuthUpdate(event) />');
            if ((features & 1) == 0) { x += addHtmlValue('', '<label><input id=dp2keep type=checkbox>' + "क्रेडेंशियल याद रखें" + '</label>'); }
            x += '</div><div id=d2keyauth style=display:none>';
            x += addHtmlValue("कुंजी फ़ाइल", '<input type=file id=dp2key style=width:230px maxlength=64 autocomplete=off onchange=sshAuthUpdate(event) />' + '<div id=d2badkey style=font-size:x-small>' + "कुंजी फ़ाइल ओपनएसएसएच प्रारूप में होनी चाहिए।" + '</div>');
            x += addHtmlValue("कुंजी पासवर्ड", '<input type=password id=dp2keypass style=width:230px maxlength=64 autocomplete=off onkeyup=sshAuthUpdate(event) />');
            if ((features & 1) == 0) {
                x += addHtmlValue('', '<label><input id=dp2keep1 type=checkbox onchange=sshAuthUpdate(event)>' + "उपयोगकर्ता और कुंजी याद रखें" + '</label>');
                x += addHtmlValue('', '<label><input id=dp2keep2 type=checkbox>' + "पासवर्ड याद" + '</label>');
            }
            x += '</div>';
            if (j.askkeypass) {
                x += '<div id=d2keyauth2 style=display:none>';
                x += addHtmlValue("कुंजिका", '<input type=password id=dp2keypass2 style=width:230px maxlength=64 autocomplete=off onkeyup=sshAuthUpdate(event) />');
                x += '</div>';
            }
            ...
            socket.onmessage = function (data) {
                ...
                if (data.data[0] == '{') {
                    ...
                    switch (json.action) {
                        ...
                        case 'autherror': { setDialogMode(2, "प्रमाणीकरण", 1, null, "प्रमाणित करने में असमर्थ।"); break; }
                        case 'sessionerror': { setDialogMode(2, "अधिवेशन", 1, null, "सत्र समाप्त हुआ।"); break; }
                        case 'sessiontimeout': { setDialogMode(2, "अधिवेशन", 1, null, "सत्र मध्यांतर।"); break; }
                    }
                } else if (data.data[0] == '~') {
