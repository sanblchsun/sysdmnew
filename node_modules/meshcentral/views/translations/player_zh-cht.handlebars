        ...
        <div id="deskarea0">
            <div id="deskarea1" class="areaHead">
                <div class="toright2">
                    ...
                    <input id="ConvertAsWebM" style="display:none" type="button" value="轉換為 WebM" onclick="saveAsWebMfile()">&nbsp;
                </div>
                <div>
                    <input id="OpenFileButton" type="button" value="打開檔案..." onclick="openfile()" style="display:none">
            ...
            <div id="deskarea4" class="areaFoot">
                ...
                <div>
                    &nbsp;
                    <input id="PlayButton" type="button" value="播放" disabled="disabled" onclick="play()">
                    <input id="PauseButton" type="button" value="暫停" disabled="disabled" onclick="pause()">
                    <input id="RestartButton" type="button" value="重新啟動" disabled="disabled" onclick="restart()">
                    <select id="PlaySpeed" onchange="this.blur();">
                        <option value="4">1/4速度</option>
                        <option value="2">1/2速</option>
    ...
    <script>
        ...
        function start() {
            ...
            browser = (function (agent) {
                switch (true) {
                    case agent.indexOf("邊緣") > -1: return "MS 邊緣 (EdgeHtml)";
                    case agent.indexOf("邊緣") > -1: return "MS 邊緣鉻";
                    case agent.indexOf("操作員") > -1 && !!window.opr: return "歌劇";
                    case agent.indexOf("Chrome") > -1 && !!window.chrome: return "Chrome";
                    case agent.indexOf("三叉戟") > -1: return "IE瀏覽器";
                    case agent.indexOf("火狐") > -1: return "火狐";
                    case agent.indexOf("蘋果瀏覽器") > -1: return "蘋果瀏覽器";
                    default: return "其他";
                }
        ...
        function processFirstBlock(type, flags, time, data) {
            ...
            if (recFileEndTime != 0) { var secs = Math.floor((recFileEndTime - time) / 1000); x += addInfo("持續時間", format("{0}秒{1}", secs, (secs > 1) ? 's' : '')); }
            x += addInfo("用戶名", recFileMetadata.username);
            x += addInfo("用戶識別碼", recFileMetadata.userid);
            x += addInfo("會話ID", recFileMetadata.sessionid);
            ...
            if (recFileMetadata.devicename) { x += addInfo("裝置名稱", recFileMetadata.devicename); }
            x += addInfo("節點ID", recFileMetadata.nodeid);
            if (recFileMetadata.protocol) {
                var p = recFileMetadata.protocol;
                if (p == 1) { p = "MeshCentral 終端"; }
                else if (p == 2) { p = "MeshCentral桌面"; }
                else if (p == 6) { p = "管理員PowerShell"; }
                else if (p == 8) { p = "用戶Shell"; }
                else if (p == 9) { p = "用戶PowerShell"; }
                else if (p == 100) { p = "Intle&reg; AMT WSMAN"; }
                else if (p == 101) { p = "Intel&reg; AMT重定向"; }
                else if ((p == 102) || (p == 200 && recFileMetadata.bpp != null)) { p = "Intel&reg; AMT KVM"; }
                else if (p == 200) { p = "MeshMessenger"; }
                x += addInfoNoEsc("協議", p);
            }
            var encQualityStr = "每像素 2 字節";
            if (recFileMetadata.bpp == 1) { encQualityStr = "每像素 1 個字節" }
            if (recFileMetadata.graymode) { if (recFileMetadata.lowcolor) { encQualityStr += ", 16 灰"; } else { encQualityStr += ", 256 灰度"; } }
            x += addInfoNoEsc("編碼質量", encQualityStr);

            if (recFileMetadata.indexInterval) {
                recFileIndexBasePtr = recFilePtr;
                x += addInfoNoEsc("尋求", format("每{0}秒索引一次", recFileMetadata.indexInterval));
            ...
            if ((recFileMetadata.protocol == 1) || (recFileMetadata.protocol == 6) || (recFileMetadata.protocol == 8) || (recFileMetadata.protocol == 9)) {
                ...
                recFileProtocol = 1;
                x += '<br /><br /><span style=color:gray>' + "按[空格]播放/暫停。" + '</span>';
                QE('PlayButton', true);
        ...
        function cleanup() {
            ...
            QV('metadatadiv', true);
            if (urlargs.stream == null) {
                QH('metadatadiv', '<span style=\"font-family:Arial,Helvetica Neue,Helvetica,sans-serif;font-size:28px\">MeshCentral Session Player</span><br /><br /><span style=color:gray>' + "拖放.mcrec檔案或單擊 “打開檔案...”" + '</span>');
            } else {
                QH('metadatadiv', '');
            }
            QV('DeskParent', true);
        ...
        function saveAsWebMfile() {
            var x = '';
            x += addHtmlValue4("框速率", '<select id=webmframerate style=width:200px><option value=100 selected>' + "10 幀/秒" + '</option><option value=50>' + "20 幀/秒" + '</option></select>');
            x += addHtmlValue4("品質", '<select id=webmquality style=width:200px><option value=90>' + "90％" + '</option><option value=80>' + "80％" + '</option><option value=60 selected>' + "60％" + '</option><option value=40>' + "40％" + '</option><option value=20>' + "20％" + '</option><option value=10>' + "10％" + '</option></select>');
            setDialogMode(2, "轉換為 WebM", 3, saveAsWebMfileEx, x);
        }
        ...
        function openfile() {
            ...
            setDialogMode(2, "打開檔案...", 3, openfileEx, x);
