<!DOCTYPE html><html dir="ltr" xmlns="http://www.w3.org/1999/xhtml"><head lang="en">
    ...
    <div id="p11" class="noselect" style="overflow:hidden">
        <div id="deskarea0" style="position:relative">
            <div id="deskarea1" class="areaHead">
                ...
                <div>
                    <input id="ConnectButton" style="display:none" type="button" value="Połącz" onclick="connectButton()">
                    <input id="DisconnectButton" type="button" value="Rozłącz" onclick="connectButton()">
    ...
    <script>
        ...
        var StatusStrs = ["Rozłączony", "Łączenie...", "Instalacja...", "Połączono"];
        var state = 0;
        var socket = null;
        var user = '';
        var pass = '';
        ...
        function sshTunnelAuthDialog(j, func) {
            var x = '';
            if (j.askkeypass) {
                x += addHtmlValue("Uwierzytelnianie", '<select id=dp2authmethod style=width:230px onchange=sshAuthUpdate(event)><option value=3 selected>' + "Przechowywany Klucz" + '</option><option value=1>' + "Nazwa użytkownika Hasło" + '</option><option value=2>' + "Nazwa użytkownika i klucz" + '</option></select>');
            } else {
                x += addHtmlValue("Uwierzytelnianie", '<select id=dp2authmethod style=width:230px onchange=sshAuthUpdate(event)><option value=1 selected>' + "Nazwa użytkownika Hasło" + '</option><option value=2>' + "Nazwa użytkownika i klucz" + '</option></select>');
            }
            x += '<div id=d2userauth style=display:none>';
            x += addHtmlValue("Nazwa użytkownika", '<input id=dp2user style=width:230px maxlength=64 autocomplete=off onkeyup=sshAuthUpdate(event) />');
            x += '</div>';
            x += '<div id=d2passauth style=display:none>';
            x += addHtmlValue("Hasło", '<input type=password id=dp2pass style=width:230px maxlength=64 autocomplete=off onkeyup=sshAuthUpdate(event) />');
            if ((features & 1) == 0) { x += addHtmlValue('', '<label><input id=dp2keep type=checkbox>' + "Zapamiętaj poświadczenia" + '</label>'); }
            x += '</div><div id=d2keyauth style=display:none>';
            x += addHtmlValue("Plik klucza", '<input type=file id=dp2key style=width:230px maxlength=64 autocomplete=off onchange=sshAuthUpdate(event) />' + '<div id=d2badkey style=font-size:x-small>' + "Plik klucza musi być w formacie OpenSSH." + '</div>');
            x += addHtmlValue("Hasło klucza", '<input type=password id=dp2keypass style=width:230px maxlength=64 autocomplete=off onkeyup=sshAuthUpdate(event) />');
            if ((features & 1) == 0) {
                x += addHtmlValue('', '<label><input id=dp2keep1 type=checkbox onchange=sshAuthUpdate(event)>' + "Zapamiętaj użytkownika i klucz" + '</label>');
                x += addHtmlValue('', '<label><input id=dp2keep2 type=checkbox>' + "Zapamiętaj hasło" + '</label>');
            }
            x += '</div>';
            if (j.askkeypass) {
                x += '<div id=d2keyauth2 style=display:none>';
                x += addHtmlValue("Hasło", '<input type=password id=dp2keypass2 style=width:230px maxlength=64 autocomplete=off onkeyup=sshAuthUpdate(event) />');
                x += '</div>';
            }
            ...
            socket.onmessage = function (data) {
                ...
                if (data.data[0] == '{') {
                    ...
                    switch (json.action) {
                        ...
                        case 'autherror': { setDialogMode(2, "Uwierzytelnianie", 1, null, "Nie można się uwierzytelnić."); break; }
                        case 'sessionerror': { setDialogMode(2, "Sesja", 1, null, "Sesja wygasła."); break; }
                        case 'sessiontimeout': { setDialogMode(2, "Sesja", 1, null, "Przekroczony czas Sesji."); break; }
                    }
                } else if (data.data[0] == '~') {
