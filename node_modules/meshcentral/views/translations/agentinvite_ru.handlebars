<!DOCTYPE html><html lang="ru"><head>
    ...
    <div id="container">
        ...
        <div id="column_l" style="overflow-y:auto">
            <h1><span id="groupname">Установка удаленного агента</span></h1>
            <p>
            ...
            <div>
                <div class="tab">
                    ...
                    <button id="tlinuxbinarytab" class="tablinks" onclick="openTab(event, 'linuxbinarytab')">Двоичный установщик Linux / BSD / macOS</button>
                    ...
                    <button id="tassistab" class="tablinks" onclick="openTab(event, 'assistab')">Ассистент</button>
                </div>

                <div id="wintab64" class="tabcontent" style="background-color:white;color:black">
                    <h3>Microsoft™ Windows 64-разрядная</h3>
                    <p><a id="win64url">Скачайте программу по этой ссылке</a>, запустите и нажмите "Инсталлировать" или "Подключиться".</p>
                ...
                <div id="wintab32" class="tabcontent" style="background-color:white;color:black">
                    <h3>Microsoft™ Windows 32-разрядная</h3>
                    <p><a id="win32url">Скачайте программу по этой ссылке</a>, запустите и нажмите "Инсталлировать" или "Подключиться".</p>
                    <div style="text-align:center">
                ...
                <div id="wintabarm64" class="tabcontent" style="background-color:white;color:black">
                    ...
                    <p><a id="winarm64url">Скачайте программу по этой ссылке</a>, запустите и нажмите "Инсталлировать" или "Подключиться".</p>
                    <div style="text-align:center">
                ...
                <div id="linuxtab" class="tabcontent" style="background-color:white;color:black">
                    <h3>Linux</h3>
                    <p>Для установки, скопируйте и вставьте следующую команду в терминал. Потребуются права root.</p>
                    <div id="linuxinstall" style="font-family:courier,'courier new',monospace;margin-left:30px"></div>
                    <input type="button" value="Скопировать в буфер обмена" style="margin-left:30px;margin-top:4px" onclick="copyToClipLinuxInstall()">
                    <p>Для удаления, скопируйте и вставьте следующую команду в терминал. Потребуются права root.</p>
                    <div id="unlinuxinstall" style="font-family:courier,'courier new',monospace;margin-left:30px"></div>
                    <input type="button" value="Скопировать в буфер обмена" style="margin-left:30px;margin-top:4px" onclick="copyToClipLinuxUnInstall()">
                    <br><br>
                </div>
                ...
                <div id="macostab64" class="tabcontent" style="background-color:white;color:black">
                    ...
                    <p><a id="macos64url">Скачайте программу по этой ссылке</a>, нажмите правой кнопкой мыши на скачанном файле, выберите пункт "Открыть" и следуйте инструкциям программы.</p>
                    <div style="text-align:center">
                ...
                <div id="macostabarm64" class="tabcontent" style="background-color:white;color:black">
                    ...
                    <p><a id="macosarm64url">Скачайте программу по этой ссылке</a>, нажмите правой кнопкой мыши на скачанном файле, выберите пункт "Открыть" и следуйте инструкциям программы.</p>
                    <div style="text-align:center">
                ...
                <div id="macostab" class="tabcontent" style="background-color:white;color:black">
                    ...
                    <p><a id="macosurl">Скачайте программу по этой ссылке</a>, нажмите правой кнопкой мыши на скачанном файле, выберите пункт "Открыть" и следуйте инструкциям программы.</p>
                    <div style="text-align:center">
                ...
                <div id="androtab" class="tabcontent" style="background-color:white;color:black">
                    ...
                    <div style="min-height:220px">
                        <p>Загрузите программное обеспечение агента с помощью магазина Google Play или APK-файла, установите его и используйте QR-код справа или ссылку для сопряжения ниже, чтобы настроить свое устройство на этом сервере..</p>
                        <p>
                        ...
                        <p>
                            <a id="androidurl">Скачать Android APK</a>
                        </p>
                        <p>
                            <a rel="noreferrer noopener" target="_blank" href="{{{magenturl}}}">Ссылка для подключения к серверу</a>
                        </p>
                    </div>
                </div>
                ...
                <div id="assistab" class="tabcontent" style="background-color:white;color:black">
                    ...
                    <div style="min-height:220px">
                        ...
                        <p>Ассистент - это инструмент на панели задач Windows, который можно использовать, когда требуется помощь..Это не агент, который работает в фоновом режиме управляемого компьютера, это инструмент, который подключается к серверу только по запросу..</p>
                        <p>
                            <a id="assisturl" rel="noreferrer noopener" target="_blank">Скачать Ассистент для Windows</a>
                        </p>
                    </div>
                    <br style="clear:both">
                </div>
    ...
    <script>
        ...
        function startup() {
            ...
            if (groupName != '') {
                QH('groupname', format("Установка агента для {0}", escapeHtml(groupName)));
                document.title = format("{0} - установка агента", groupName);
            } else {
                document.title = "Установка агента";
            }
        ...
        function linuxBinarySetup() {
            var x = '<h3>' + "Двоичный установщик Linux / BSD / macOS" + '</h3>', moreoptions = '';
            var opts = '<select id=aginsSelect onchange=addAgentToMeshClick() style=width:236px>';
            opts += '<option value=5 selected>' + "Двоичный установщик Linux / BSD / macOS" + '</option>';
            opts += '</select>';
            ...
            x += addHtmlValue("Тип системы", '<select id=aginsSysType onchange=addAgentToMeshClick() style=width:236px>' + moreoptions + '</select>');
            x += '</div>';
            x += '<div id=aginsTypeDiv>';
            x += addHtmlValue("Тип установки", '<select id=aginsType onchange=addAgentToMeshClick() style=width:236px><option value=0>' + "Фоновый и интерактивный" + '</option><option value=2>' + "Только фоновый" + '</option><option value=1>' + "Только интерактивный режим" + '</option></select>');
            x += '</div><hr>';
            ...
            x += addHtmlValue("Mesh Agent", '<a id=aginsbinlnk name="meshagents?id=' + meshid + '&installflags=0' + (urlargs.key?('&key=' + urlargs.key):'') + '">' + "MeshAgent" + '</a> <img src=images/link4.png height=10 width=10 title="' + "Скопировать URL агента в буфер обмена" + '" style=cursor:pointer onclick=copyAgentUrl("meshagents?id=' + meshid + '&installflags=",1)>');
            x += addHtmlValue("Команда", '<input id=aginsbincmd type=text style="width:216px" readonly value=\'wget -O meshagent ' + ((serverNoProxy != 0) ?'--no-proxy':'') + ' --no-check-certificate \"https://' + servername + portStr + domainUrl + 'meshagents?id=' + meshid.split('$').join('%24').split('@').join('%40') + '\' /> <img src=images/link4.png height=10 width=10 title="' + "Скопировать URL агента в буфер обмена" + '" style=cursor:pointer onclick=copyAgentIdValue("aginsbincmd")>');
            x += '</div>';
            QH('linuxbinarytab', x);
            addAgentToMeshClick();
        }
