<!DOCTYPE html><html dir="ltr" xmlns="http://www.w3.org/1999/xhtml"><head lang="en">
    ...
    </head><body style="overflow:hidden;background-color:black" onload="start()">{{{customCSSTags}}}
    ...
    <div id="p11" class="noselect" style="overflow:hidden">
        <div id="deskarea0" style="position:relative">
            <div id="deskarea1" class="areaHead">
                ...
                <div>
                    <input id="ConnectButton" style="display:none" type="button" value="Bağlan" onclick="connectButton()">
                    <input id="DisconnectButton" type="button" value="Bağlantıyı kes" onclick="connectButton()">
    ...
    <script>
        ...
        var StatusStrs = ["Bağlantı kesildi", "Bağlanıyor...", "Kurmak...", "Bağlandı"];
        var state = 0;
        var socket = null;
        var user = '';
        var pass = '';
        ...
        function sshTunnelAuthDialog(j, func) {
            var x = '';
            if (j.askkeypass) {
                x += addHtmlValue("Kimlik doğrulama", '<select id=dp2authmethod style=width:230px onchange=sshAuthUpdate(event)><option value=3 selected>' + "Saklanan Anahtar" + '</option><option value=1>' + "Kullanıcı adı Şifre" + '</option><option value=2>' + "Kullanıcı adı ve Anahtar" + '</option></select>');
            } else {
                x += addHtmlValue("Kimlik doğrulama", '<select id=dp2authmethod style=width:230px onchange=sshAuthUpdate(event)><option value=1 selected>' + "Kullanıcı adı Şifre" + '</option><option value=2>' + "Kullanıcı adı ve Anahtar" + '</option></select>');
            }
            x += '<div id=d2userauth style=display:none>';
            x += addHtmlValue("Kullanıcı adı", '<input id=dp2user style=width:230px maxlength=64 autocomplete=off onkeyup=sshAuthUpdate(event) />');
            x += '</div>';
            x += '<div id=d2passauth style=display:none>';
            x += addHtmlValue("Parola", '<input type=password id=dp2pass style=width:230px maxlength=64 autocomplete=off onkeyup=sshAuthUpdate(event) />');
            if ((features & 1) == 0) { x += addHtmlValue('', '<label><input id=dp2keep type=checkbox>' + "Kimlik bilgilerini hatırla" + '</label>'); }
            x += '</div><div id=d2keyauth style=display:none>';
            x += addHtmlValue("Anahtar dosya", '<input type=file id=dp2key style=width:230px maxlength=64 autocomplete=off onchange=sshAuthUpdate(event) />' + '<div id=d2badkey style=font-size:x-small>' + "Anahtar dosyası OpenSSH formatında olmalıdır." + '</div>');
            x += addHtmlValue("Anahtar Parola", '<input type=password id=dp2keypass style=width:230px maxlength=64 autocomplete=off onkeyup=sshAuthUpdate(event) />');
            if ((features & 1) == 0) {
                x += addHtmlValue('', '<label><input id=dp2keep1 type=checkbox onchange=sshAuthUpdate(event)>' + "Kullanıcıyı ve anahtarı hatırla" + '</label>');
                x += addHtmlValue('', '<label><input id=dp2keep2 type=checkbox>' + "Şifre hatırlamak" + '</label>');
            }
            x += '</div>';
            if (j.askkeypass) {
                x += '<div id=d2keyauth2 style=display:none>';
                x += addHtmlValue("Parola", '<input type=password id=dp2keypass2 style=width:230px maxlength=64 autocomplete=off onkeyup=sshAuthUpdate(event) />');
                x += '</div>';
            }
            ...
            socket.onmessage = function (data) {
                ...
                if (data.data[0] == '{') {
                    ...
                    switch (json.action) {
                        ...
                        case 'autherror': { setDialogMode(2, "Kimlik doğrulama", 1, null, "Kimlik doğrulanamadı."); break; }
                        case 'sessionerror': { setDialogMode(2, "Oturum, toplantı, celse", 1, null, "Oturum süresi doldu."); break; }
                        case 'sessiontimeout': { setDialogMode(2, "Oturum, toplantı, celse", 1, null, "Oturum zaman aşımına uğradı."); break; }
                    }
                } else if (data.data[0] == '~') {
