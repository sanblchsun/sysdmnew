        ...
        <div id="deskarea0">
            <div id="deskarea1" class="areaHead">
                <div class="toright2">
                    ...
                    <input id="ConvertAsWebM" style="display:none" type="button" value="Konverter til WebM" onclick="saveAsWebMfile()">&nbsp;
                </div>
                <div>
                    <input id="OpenFileButton" type="button" value="Åben fil..." onclick="openfile()" style="display:none">
            ...
            <div id="deskarea4" class="areaFoot">
                ...
                <div>
                    &nbsp;
                    <input id="PlayButton" type="button" value="Afspil" disabled="disabled" onclick="play()">
                    <input id="PauseButton" type="button" value="Pause" disabled="disabled" onclick="pause()">
                    <input id="RestartButton" type="button" value="Genstart" disabled="disabled" onclick="restart()">
                    <select id="PlaySpeed" onchange="this.blur();">
                        <option value="4">1/4 hastighed</option>
                        <option value="2">1/2 hastighed</option>
                        <option value="1" selected="">Normal hastighed</option>
                        <option value="0.5">2x hastighed</option>
                        <option value="0.25">4x hastighed</option>
                        <option value="0.1">10x hastighed</option>
                    </select>
    ...
    <script>
        ...
        function start() {
            ...
            if (urlargs.stream != null) {
                QV('metadatadiv', true);
                QH('metadatadiv', "Forbinder til serveren...");
        ...
        function processFirstBlock(type, flags, time, data) {
            ...
            if (recFileEndTime != 0) { var secs = Math.floor((recFileEndTime - time) / 1000); x += addInfo("Varighed", format("{0} sekund{1}", secs, (secs > 1) ? 's' : '')); }
            x += addInfo("Brugernavn", recFileMetadata.username);
            x += addInfo("BrugerID", recFileMetadata.userid);
            x += addInfo("SessionID", recFileMetadata.sessionid);
            if (recFileMetadata.ipaddr1 && recFileMetadata.ipaddr2) { x += addInfo("Adresser", format("{0} til {1}", recFileMetadata.ipaddr1, recFileMetadata.ipaddr2)); }
            if (recFileMetadata.devicename) { x += addInfo("Enhedsnavn", recFileMetadata.devicename); }
            ...
            if (recFileMetadata.protocol) {
                ...
                else if (p == 8) { p = "Bruger Shell"; }
                else if (p == 9) { p = "Bruger PowerShell"; }
            ...
            var encQualityStr = "2 byte pr. pixel";
            if (recFileMetadata.bpp == 1) { encQualityStr = "1 byte pr. pixel" }
            if (recFileMetadata.graymode) { if (recFileMetadata.lowcolor) { encQualityStr += ", 16 grå"; } else { encQualityStr += ", 256 grå"; } }
            x += addInfoNoEsc("Kodningskvalitet", encQualityStr);

            if (recFileMetadata.indexInterval) {
                recFileIndexBasePtr = recFilePtr;
                x += addInfoNoEsc("Søger", format("Indekseres hvert {0} sekund", recFileMetadata.indexInterval));
            ...
            if ((recFileMetadata.protocol == 1) || (recFileMetadata.protocol == 6) || (recFileMetadata.protocol == 8) || (recFileMetadata.protocol == 9)) {
                ...
                recFileProtocol = 1;
                x += '<br /><br /><span style=color:gray>' + "Tryk på [mellemrum] for at afspille/pause." + '</span>';
                QE('PlayButton', true);
        ...
        function cleanup() {
            ...
            QV('metadatadiv', true);
            if (urlargs.stream == null) {
                QH('metadatadiv', '<span style=\"font-family:Arial,Helvetica Neue,Helvetica,sans-serif;font-size:28px\">MeshCentral Session Player</span><br /><br /><span style=color:gray>' + "Træk og slip en .mcrec-fil, eller klik på \"Åbn fil...\"" + '</span>');
            } else {
                QH('metadatadiv', '');
            }
            QV('DeskParent', true);
        ...
        function saveAsWebMfile() {
            var x = '';
            x += addHtmlValue4("Billedhastighed", '<select id=webmframerate style=width:200px><option value=100 selected>' + "10 billeder/sek" + '</option><option value=50>' + "20 billeder/sek" + '</option></select>');
            ...
            setDialogMode(2, "Konverter til WebM", 3, saveAsWebMfileEx, x);
        }
        ...
        function openfile() {
            ...
            setDialogMode(2, "Åben fil...", 3, openfileEx, x);
