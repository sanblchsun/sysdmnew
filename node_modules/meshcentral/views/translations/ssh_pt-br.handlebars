<!DOCTYPE html><html dir="ltr" xmlns="http://www.w3.org/1999/xhtml"><head lang="en">
    ...
    <script>
        ...
        var StatusStrs = ["Desconectado", "Conectando ...", "Configurando...", "Conectado"];
        var state = 0;
        var socket = null;
        var user = '';
        var pass = '';
        ...
        function sshTunnelAuthDialog(j, func) {
            var x = '';
            if (j.askkeypass) {
                x += addHtmlValue("Autenticação", '<select id=dp2authmethod style=width:230px onchange=sshAuthUpdate(event)><option value=3 selected>' + "Chave Armazenada" + '</option><option value=1>' + "Usuario Contraseña" + '</option><option value=2>' + "Nombre de usuario y clave" + '</option></select>');
            } else {
                x += addHtmlValue("Autenticação", '<select id=dp2authmethod style=width:230px onchange=sshAuthUpdate(event)><option value=1 selected>' + "Usuario Contraseña" + '</option><option value=2>' + "Nombre de usuario y clave" + '</option></select>');
            }
            x += '<div id=d2userauth style=display:none>';
            x += addHtmlValue("Nome do usuário", '<input id=dp2user style=width:230px maxlength=64 autocomplete=off onkeyup=sshAuthUpdate(event) />');
            x += '</div>';
            x += '<div id=d2passauth style=display:none>';
            x += addHtmlValue("Senha", '<input type=password id=dp2pass style=width:230px maxlength=64 autocomplete=off onkeyup=sshAuthUpdate(event) />');
            if ((features & 1) == 0) { x += addHtmlValue('', '<label><input id=dp2keep type=checkbox>' + "Lembrar credenciais" + '</label>'); }
            ...
            if ((features & 1) == 0) {
                x += addHtmlValue('', '<label><input id=dp2keep1 type=checkbox onchange=sshAuthUpdate(event)>' + "Lembrar usuário e chave" + '</label>');
                x += addHtmlValue('', '<label><input id=dp2keep2 type=checkbox>' + "Lembrar senha" + '</label>');
            }
            x += '</div>';
            if (j.askkeypass) {
                x += '<div id=d2keyauth2 style=display:none>';
                x += addHtmlValue("Senha", '<input type=password id=dp2keypass2 style=width:230px maxlength=64 autocomplete=off onkeyup=sshAuthUpdate(event) />');
                x += '</div>';
            }
            ...
            socket.onmessage = function (data) {
                ...
                if (data.data[0] == '{') {
                    ...
                    switch (json.action) {
                        ...
                        case 'autherror': { setDialogMode(2, "Autenticação", 1, null, "Não foi possível autenticar."); break; }
                        case 'sessionerror': { setDialogMode(2, "Sessão", 1, null, "Sessão expirada."); break; }
                        case 'sessiontimeout': { setDialogMode(2, "Sessão", 1, null, "Sessão expirada."); break; }
                    }
                } else if (data.data[0] == '~') {
