<!DOCTYPE html><html dir="ltr" xmlns="http://www.w3.org/1999/xhtml"><head lang="en">
    ...
    <div id="p11" class="noselect" style="overflow:hidden">
        <div id="deskarea0" style="position:relative">
            <div id="deskarea1" class="areaHead">
                ...
                <div>
                    <input id="ConnectButton" style="display:none" type="button" value="Verbinden" onclick="connectButton()">
                    <input id="DisconnectButton" type="button" value="Trennen" onclick="connectButton()">
    ...
    <script>
        ...
        var StatusStrs = ["Getrennt", "Verbinden...", "Einrichtung...", "Verbunden"];
        var state = 0;
        var socket = null;
        var user = '';
        var pass = '';
        ...
        function sshTunnelAuthDialog(j, func) {
            var x = '';
            if (j.askkeypass) {
                x += addHtmlValue("Authentifizierung", '<select id=dp2authmethod style=width:230px onchange=sshAuthUpdate(event)><option value=3 selected>' + "Gespeicherter Schlüssel" + '</option><option value=1>' + "Benutzername Passwort" + '</option><option value=2>' + "Benutzername und Schlüssel" + '</option></select>');
            } else {
                x += addHtmlValue("Authentifizierung", '<select id=dp2authmethod style=width:230px onchange=sshAuthUpdate(event)><option value=1 selected>' + "Benutzername Passwort" + '</option><option value=2>' + "Benutzername und Schlüssel" + '</option></select>');
            }
            x += '<div id=d2userauth style=display:none>';
            x += addHtmlValue("Benutzername", '<input id=dp2user style=width:230px maxlength=64 autocomplete=off onkeyup=sshAuthUpdate(event) />');
            x += '</div>';
            x += '<div id=d2passauth style=display:none>';
            x += addHtmlValue("Passwort", '<input type=password id=dp2pass style=width:230px maxlength=64 autocomplete=off onkeyup=sshAuthUpdate(event) />');
            if ((features & 1) == 0) { x += addHtmlValue('', '<label><input id=dp2keep type=checkbox>' + "Anmeldedaten merken" + '</label>'); }
            x += '</div><div id=d2keyauth style=display:none>';
            x += addHtmlValue("Schlüsseldatei", '<input type=file id=dp2key style=width:230px maxlength=64 autocomplete=off onchange=sshAuthUpdate(event) />' + '<div id=d2badkey style=font-size:x-small>' + "Die Schlüsseldatei muss im OpenSSH-Format vorliegen." + '</div>');
            x += addHtmlValue("Schlüssel Passwort", '<input type=password id=dp2keypass style=width:230px maxlength=64 autocomplete=off onkeyup=sshAuthUpdate(event) />');
            if ((features & 1) == 0) {
                x += addHtmlValue('', '<label><input id=dp2keep1 type=checkbox onchange=sshAuthUpdate(event)>' + "Benutzer & Schlüssel merken" + '</label>');
                x += addHtmlValue('', '<label><input id=dp2keep2 type=checkbox>' + "Passwort merken" + '</label>');
            }
            x += '</div>';
            if (j.askkeypass) {
                x += '<div id=d2keyauth2 style=display:none>';
                x += addHtmlValue("Passwort", '<input type=password id=dp2keypass2 style=width:230px maxlength=64 autocomplete=off onkeyup=sshAuthUpdate(event) />');
                x += '</div>';
            }
            ...
            socket.onmessage = function (data) {
                ...
                if (data.data[0] == '{') {
                    ...
                    switch (json.action) {
                        ...
                        case 'autherror': { setDialogMode(2, "Authentifizierung", 1, null, "Nicht in der Lage zu authentifizieren."); break; }
                        case 'sessionerror': { setDialogMode(2, "Sitzung", 1, null, "Sitzung abgelaufen."); break; }
                        case 'sessiontimeout': { setDialogMode(2, "Sitzung", 1, null, "Session-Timeout."); break; }
                    }
                } else if (data.data[0] == '~') {
