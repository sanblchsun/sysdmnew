        ...
        <div id="deskarea0">
            <div id="deskarea1" class="areaHead">
                <div class="toright2">
                    ...
                    <input id="ConvertAsWebM" style="display:none" type="button" value="转换为 WebM" onclick="saveAsWebMfile()">&nbsp;
                </div>
                <div>
                    <input id="OpenFileButton" type="button" value="打开文件..." onclick="openfile()" style="display:none">
            ...
            <div id="deskarea4" class="areaFoot">
                ...
                <div>
                    ...
                    <input id="PauseButton" type="button" value="暂停" disabled="disabled" onclick="pause()">
                    <input id="RestartButton" type="button" value="重新启动" disabled="disabled" onclick="restart()">
    ...
    <script>
        ...
        function start() {
            ...
            browser = (function (agent) {
                switch (true) {
                    case agent.indexOf("边缘") > -1: return "MS 边缘 (EdgeHtml)";
                    case agent.indexOf("边缘") > -1: return "MS Edge 铬";
                    case agent.indexOf("操作") > -1 && !!window.opr: return "歌剧";
                    case agent.indexOf("Chrome") > -1 && !!window.chrome: return "Chrome";
                    case agent.indexOf("三叉戟") > -1: return "IE浏览器";
                    case agent.indexOf("火狐") > -1: return "火狐";
                    case agent.indexOf("苹果浏览器") > -1: return "苹果浏览器";
                    default: return "其他";
                }
        ...
        function processFirstBlock(type, flags, time, data) {
            ...
            if (recFileEndTime != 0) { var secs = Math.floor((recFileEndTime - time) / 1000); x += addInfo("持续时间", format("{0}秒{1}", secs, (secs > 1) ? 's' : '')); }
            x += addInfo("用户名", recFileMetadata.username);
            x += addInfo("用户识别码", recFileMetadata.userid);
            ...
            if (recFileMetadata.devicename) { x += addInfo("设备名称", recFileMetadata.devicename); }
            x += addInfo("节点ID", recFileMetadata.nodeid);
            if (recFileMetadata.protocol) {
                var p = recFileMetadata.protocol;
                if (p == 1) { p = "MeshCentral 终端"; }
                else if (p == 2) { p = "MeshCentral桌面"; }
                else if (p == 6) { p = "管理员PowerShell"; }
                else if (p == 8) { p = "用户Shell"; }
                else if (p == 9) { p = "用户PowerShell"; }
                else if (p == 100) { p = "英特尔&reg;AMT WSMAN"; }
                else if (p == 101) { p = "英特尔&reg;AMT重定向"; }
                else if ((p == 102) || (p == 200 && recFileMetadata.bpp != null)) { p = "英特尔&reg;AMT KVM"; }
                else if (p == 200) { p = "MeshMessenger"; }
                x += addInfoNoEsc("协议", p);
            }
            var encQualityStr = "每像素 2 字节";
            if (recFileMetadata.bpp == 1) { encQualityStr = "每像素 1 个字节" }
            if (recFileMetadata.graymode) { if (recFileMetadata.lowcolor) { encQualityStr += ", 16 灰"; } else { encQualityStr += ", 256 灰度"; } }
            x += addInfoNoEsc("编码质量", encQualityStr);

            if (recFileMetadata.indexInterval) {
                recFileIndexBasePtr = recFilePtr;
                x += addInfoNoEsc("寻求", format("每{0}秒索引一次", recFileMetadata.indexInterval));
        ...
        function cleanup() {
            ...
            QV('metadatadiv', true);
            if (urlargs.stream == null) {
                QH('metadatadiv', '<span style=\"font-family:Arial,Helvetica Neue,Helvetica,sans-serif;font-size:28px\">MeshCentral Session Player</span><br /><br /><span style=color:gray>' + "拖放.mcrec文件或单击 “打开文件...”" + '</span>');
            } else {
                QH('metadatadiv', '');
            }
            QV('DeskParent', true);
        ...
        function saveAsWebMfile() {
            var x = '';
            x += addHtmlValue4("贞速率", '<select id=webmframerate style=width:200px><option value=100 selected>' + "10 帧/秒" + '</option><option value=50>' + "20 帧/秒" + '</option></select>');
            x += addHtmlValue4("质量", '<select id=webmquality style=width:200px><option value=90>' + "90％" + '</option><option value=80>' + "80％" + '</option><option value=60 selected>' + "60％" + '</option><option value=40>' + "40％" + '</option><option value=20>' + "20％" + '</option><option value=10>' + "10％" + '</option></select>');
            setDialogMode(2, "转换为 WebM", 3, saveAsWebMfileEx, x);
        }
        ...
        function openfile() {
            ...
            setDialogMode(2, "打开文件...", 3, openfileEx, x);
