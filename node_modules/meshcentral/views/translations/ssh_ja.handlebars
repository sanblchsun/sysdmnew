<!DOCTYPE html><html dir="ltr" xmlns="http://www.w3.org/1999/xhtml"><head lang="en">
    ...
    <div id="p11" class="noselect" style="overflow:hidden">
        <div id="deskarea0" style="position:relative">
            <div id="deskarea1" class="areaHead">
                ...
                <div>
                    <input id="ConnectButton" style="display:none" type="button" value="接続" onclick="connectButton()">
                    <input id="DisconnectButton" type="button" value="切断" onclick="connectButton()">
    ...
    <script>
        ...
        var StatusStrs = ["切断されました", "接続しています...", "セットアップ...", "接続済み"];
        var state = 0;
        var socket = null;
        var user = '';
        var pass = '';
        ...
        function sshTunnelAuthDialog(j, func) {
            var x = '';
            if (j.askkeypass) {
                x += addHtmlValue("認証", '<select id=dp2authmethod style=width:230px onchange=sshAuthUpdate(event)><option value=3 selected>' + "ストアされた鍵" + '</option><option value=1>' + "ユーザー名パスワード" + '</option><option value=2>' + "ユーザー名とキー" + '</option></select>');
            } else {
                x += addHtmlValue("認証", '<select id=dp2authmethod style=width:230px onchange=sshAuthUpdate(event)><option value=1 selected>' + "ユーザー名パスワード" + '</option><option value=2>' + "ユーザー名とキー" + '</option></select>');
            }
            x += '<div id=d2userauth style=display:none>';
            x += addHtmlValue("ユーザー名", '<input id=dp2user style=width:230px maxlength=64 autocomplete=off onkeyup=sshAuthUpdate(event) />');
            x += '</div>';
            x += '<div id=d2passauth style=display:none>';
            x += addHtmlValue("パスワード", '<input type=password id=dp2pass style=width:230px maxlength=64 autocomplete=off onkeyup=sshAuthUpdate(event) />');
            if ((features & 1) == 0) { x += addHtmlValue('', '<label><input id=dp2keep type=checkbox>' + "クレデンシャルを記憶" + '</label>'); }
            x += '</div><div id=d2keyauth style=display:none>';
            x += addHtmlValue("キーファイル", '<input type=file id=dp2key style=width:230px maxlength=64 autocomplete=off onchange=sshAuthUpdate(event) />' + '<div id=d2badkey style=font-size:x-small>' + "キーファイルはOpenSSH形式である必要があります。" + '</div>');
            x += addHtmlValue("キーパスワード", '<input type=password id=dp2keypass style=width:230px maxlength=64 autocomplete=off onkeyup=sshAuthUpdate(event) />');
            if ((features & 1) == 0) {
                x += addHtmlValue('', '<label><input id=dp2keep1 type=checkbox onchange=sshAuthUpdate(event)>' + "ユーザーと鍵を記憶" + '</label>');
                x += addHtmlValue('', '<label><input id=dp2keep2 type=checkbox>' + "パスワードを記憶" + '</label>');
            }
            x += '</div>';
            if (j.askkeypass) {
                x += '<div id=d2keyauth2 style=display:none>';
                x += addHtmlValue("パスワード", '<input type=password id=dp2keypass2 style=width:230px maxlength=64 autocomplete=off onkeyup=sshAuthUpdate(event) />');
                x += '</div>';
            }
            ...
            socket.onmessage = function (data) {
                ...
                if (data.data[0] == '{') {
                    ...
                    switch (json.action) {
                        ...
                        case 'autherror': { setDialogMode(2, "認証", 1, null, "認証できません。"); break; }
                        case 'sessionerror': { setDialogMode(2, "セッション", 1, null, "セッションの有効期限が切れ。"); break; }
                        case 'sessiontimeout': { setDialogMode(2, "セッション", 1, null, "セッションタイムアウト。"); break; }
                    }
                } else if (data.data[0] == '~') {
