<!DOCTYPE html><html lang="fr" dir="ltr" xmlns="http://www.w3.org/1999/xhtml"><head>
        ...
        <div id="deskarea0">
            <div id="deskarea1" class="areaHead">
                <div class="toright2">
                    ...
                    <input id="ConvertAsWebM" style="display:none" type="button" value="Convertir au format WebM" onclick="saveAsWebMfile()">&nbsp;
                </div>
                <div>
                    <input id="OpenFileButton" type="button" value="Ouvrir le fichier..." onclick="openfile()" style="display:none">
    ...
    <script>
        ...
        function start() {
            ...
            browser = (function (agent) {
                switch (true) {
                    ...
                    case agent.indexOf("bord") > -1: return "MS Edge Chromium";
                    case agent.indexOf("opr") > -1 && !!window.opr: return "Opéra";
            ...
            if (urlargs.stream != null) {
                QV('metadatadiv', true);
                QH('metadatadiv', "Connexion au serveur ...");
        ...
        function processFirstBlock(type, flags, time, data) {
            ...
            if (recFileEndTime != 0) { var secs = Math.floor((recFileEndTime - time) / 1000); x += addInfo("Durée", format("{0} seconde{1}", secs, (secs > 1) ? 's' : '')); }
            ...
            if (recFileMetadata.ipaddr1 && recFileMetadata.ipaddr2) { x += addInfo("Adresses", format("{0} à {1}", recFileMetadata.ipaddr1, recFileMetadata.ipaddr2)); }
            if (recFileMetadata.devicename) { x += addInfo("Nom de l'appareil", recFileMetadata.devicename); }
            ...
            if (recFileMetadata.protocol) {
                ...
                else if (p == 2) { p = "Bureau MeshCentral"; }
                else if (p == 6) { p = "Powershell administateur"; }
                else if (p == 8) { p = "Shell utilisateur"; }
                else if (p == 9) { p = "PowerShell utilisateur"; }
                else if (p == 100) { p = "Intel&reg; AMT WSMAN"; }
                else if (p == 101) { p = "Redirection Intel&reg; AMT"; }
                else if ((p == 102) || (p == 200 && recFileMetadata.bpp != null)) { p = "Intel&reg; AMT KVM"; }
                else if (p == 200) { p = "MeshMessenger"; }
                x += addInfoNoEsc("Protocole", p);
            }
            var encQualityStr = "2 octects par pixel";
            if (recFileMetadata.bpp == 1) { encQualityStr = "1 octect par pixel" }
            if (recFileMetadata.graymode) { if (recFileMetadata.lowcolor) { encQualityStr += ", 16 gris"; } else { encQualityStr += ", 256 gris"; } }
            x += addInfoNoEsc("Qualité d'encodage", encQualityStr);

            if (recFileMetadata.indexInterval) {
                recFileIndexBasePtr = recFilePtr;
                x += addInfoNoEsc("Cherchant", format("Indexé toutes les {0} seconde(s)", recFileMetadata.indexInterval));
        ...
        function processBlockEx(type, flags, time, data, forced) {
            ...
            if ((type == 3) && (recFileProtocol == 102)) {
                var tile = new Image();
                tile.src = "données: image / png; base64," + btoa(data);
        ...
        function cleanup() {
            ...
            QV('metadatadiv', true);
            if (urlargs.stream == null) {
                QH('metadatadiv', '<span style=\"font-family:Arial,Helvetica Neue,Helvetica,sans-serif;font-size:28px\">MeshCentral Session Player</span><br /><br /><span style=color:gray>' + "Glisser & Déposer un fichier .mcrec ou cliquer sur \"Ouvrir fichier...\"" + '</span>');
            } else {
                QH('metadatadiv', '');
            }
            QV('DeskParent', true);
        ...
        function saveAsWebMfile() {
            var x = '';
            x += addHtmlValue4("Taux de rafraîchissement", '<select id=webmframerate style=width:200px><option value=100 selected>' + "10 images/s" + '</option><option value=50>' + "20 images/s" + '</option></select>');
            x += addHtmlValue4("Qualité", '<select id=webmquality style=width:200px><option value=90>' + "90%" + '</option><option value=80>' + "80%" + '</option><option value=60 selected>' + "60%" + '</option><option value=40>' + "40%" + '</option><option value=20>' + "20%" + '</option><option value=10>' + "10%" + '</option></select>');
            setDialogMode(2, "Convertir au format WebM", 3, saveAsWebMfileEx, x);
        }
        ...
        function openfile() {
            ...
            setDialogMode(2, "Ouvrir le fichier...", 3, openfileEx, x);
