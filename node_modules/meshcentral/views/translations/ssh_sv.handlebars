<!DOCTYPE html><html dir="ltr" xmlns="http://www.w3.org/1999/xhtml"><head lang="en">
    ...
    <div id="p11" class="noselect" style="overflow:hidden">
        <div id="deskarea0" style="position:relative">
            <div id="deskarea1" class="areaHead">
                ...
                <div>
                    <input id="ConnectButton" style="display:none" type="button" value="Anslut" onclick="connectButton()">
                    <input id="DisconnectButton" type="button" value="Koppla ifrån" onclick="connectButton()">
    ...
    <script>
        ...
        var StatusStrs = ["Frånkopplad", "Ansluter...", "Uppstart...", "Ansluten"];
        var state = 0;
        var socket = null;
        var user = '';
        var pass = '';
        ...
        function sshTunnelAuthDialog(j, func) {
            var x = '';
            if (j.askkeypass) {
                x += addHtmlValue("Autentisering", '<select id=dp2authmethod style=width:230px onchange=sshAuthUpdate(event)><option value=3 selected>' + "Lagrad nyckel" + '</option><option value=1>' + "Användarnamn Lösenord" + '</option><option value=2>' + "Användarnamn och nyckel" + '</option></select>');
            } else {
                x += addHtmlValue("Autentisering", '<select id=dp2authmethod style=width:230px onchange=sshAuthUpdate(event)><option value=1 selected>' + "Användarnamn Lösenord" + '</option><option value=2>' + "Användarnamn och nyckel" + '</option></select>');
            }
            x += '<div id=d2userauth style=display:none>';
            x += addHtmlValue("Användarnamn", '<input id=dp2user style=width:230px maxlength=64 autocomplete=off onkeyup=sshAuthUpdate(event) />');
            x += '</div>';
            x += '<div id=d2passauth style=display:none>';
            x += addHtmlValue("Lösenord", '<input type=password id=dp2pass style=width:230px maxlength=64 autocomplete=off onkeyup=sshAuthUpdate(event) />');
            if ((features & 1) == 0) { x += addHtmlValue('', '<label><input id=dp2keep type=checkbox>' + "Kom ihåg referenser" + '</label>'); }
            x += '</div><div id=d2keyauth style=display:none>';
            x += addHtmlValue("Nyckelfil", '<input type=file id=dp2key style=width:230px maxlength=64 autocomplete=off onchange=sshAuthUpdate(event) />' + '<div id=d2badkey style=font-size:x-small>' + "Nyckelfilen måste vara i OpenSSH -format." + '</div>');
            x += addHtmlValue("Nyckel lösenord", '<input type=password id=dp2keypass style=width:230px maxlength=64 autocomplete=off onkeyup=sshAuthUpdate(event) />');
            if ((features & 1) == 0) {
                x += addHtmlValue('', '<label><input id=dp2keep1 type=checkbox onchange=sshAuthUpdate(event)>' + "Kom ihåg användare och nyckel" + '</label>');
                x += addHtmlValue('', '<label><input id=dp2keep2 type=checkbox>' + "Kom ihåg lösenord" + '</label>');
            }
            x += '</div>';
            if (j.askkeypass) {
                x += '<div id=d2keyauth2 style=display:none>';
                x += addHtmlValue("Lösenord", '<input type=password id=dp2keypass2 style=width:230px maxlength=64 autocomplete=off onkeyup=sshAuthUpdate(event) />');
                x += '</div>';
            }
            ...
            socket.onmessage = function (data) {
                ...
                if (data.data[0] == '{') {
                    ...
                    switch (json.action) {
                        ...
                        case 'autherror': { setDialogMode(2, "Autentisering", 1, null, "Det gick inte att autentisera."); break; }
                        case 'sessionerror': { setDialogMode(2, "Session", 1, null, "Sessionen löpte."); break; }
                        case 'sessiontimeout': { setDialogMode(2, "Session", 1, null, "Sessionen har gått ut."); break; }
                    }
                } else if (data.data[0] == '~') {
