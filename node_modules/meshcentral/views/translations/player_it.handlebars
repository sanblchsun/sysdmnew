        ...
        <div id="deskarea0">
            <div id="deskarea1" class="areaHead">
                <div class="toright2">
                    ...
                    <input id="ConvertAsWebM" style="display:none" type="button" value="Converti in WebM" onclick="saveAsWebMfile()">&nbsp;
                </div>
                <div>
                    <input id="OpenFileButton" type="button" value="Apri il file..." onclick="openfile()" style="display:none">
    ...
    <script>
        ...
        function start() {
            ...
            browser = (function (agent) {
                switch (true) {
                    case agent.indexOf("bordo") > -1: return "MS Edge (EdgeHtml)";
                    case agent.indexOf("edg") > -1: return "MS Edge Chromium";
                    case agent.indexOf("opr") > -1 && !!window.opr: return "musica lirica";
                    case agent.indexOf("Chrome") > -1 && !!window.chrome: return "Chrome";
                    case agent.indexOf("tridente") > -1: return "Internet Explorer";
            ...
            if (urlargs.stream != null) {
                QV('metadatadiv', true);
                QH('metadatadiv', "Connessione al server...");
        ...
        function processFirstBlock(type, flags, time, data) {
            ...
            if (recFileEndTime != 0) { var secs = Math.floor((recFileEndTime - time) / 1000); x += addInfo("Durata", format("{0} secondo{1}", secs, (secs > 1) ? 's' : '')); }
            x += addInfo("Nome utente", recFileMetadata.username);
            x += addInfo("ID utente", recFileMetadata.userid);
            x += addInfo("ID sessione", recFileMetadata.sessionid);
            if (recFileMetadata.ipaddr1 && recFileMetadata.ipaddr2) { x += addInfo("Indirizzi", format("{0} a {1}", recFileMetadata.ipaddr1, recFileMetadata.ipaddr2)); }
            if (recFileMetadata.devicename) { x += addInfo("Nome del dispositivo", recFileMetadata.devicename); }
            ...
            if (recFileMetadata.protocol) {
                ...
                else if (p == 6) { p = "Credenziali di amministratore PowerShell"; }
                else if (p == 8) { p = "Shell utente "; }
                else if (p == 9) { p = "PowerShell Utente "; }
                else if (p == 100) { p = "Intel&reg; AMT WSMAN"; }
                else if (p == 101) { p = "Intel&reg; Reindirizzamento AMT"; }
            ...
            if (recFileMetadata.graymode) { if (recFileMetadata.lowcolor) { encQualityStr += ", 16 grigi"; } else { encQualityStr += ", 256 grigi"; } }
            x += addInfoNoEsc("Qualità di codifica", encQualityStr);

            if (recFileMetadata.indexInterval) {
                recFileIndexBasePtr = recFilePtr;
                x += addInfoNoEsc("in cerca di", format("Indicizzato ogni {0} secondi", recFileMetadata.indexInterval));
            ...
            if ((recFileMetadata.protocol == 1) || (recFileMetadata.protocol == 6) || (recFileMetadata.protocol == 8) || (recFileMetadata.protocol == 9)) {
                ...
                recFileProtocol = 1;
                x += '<br /><br /><span style=color:gray>' + "Premi [spazio] per riprodurre/mettere in pausa." + '</span>';
                QE('PlayButton', true);
        ...
        function processBlockEx(type, flags, time, data, forced) {
            ...
            if ((type == 3) && (recFileProtocol == 102)) {
                var tile = new Image();
                tile.src = "dati:immagine/png;base64," + btoa(data);
        ...
        function cleanup() {
            ...
            QV('metadatadiv', true);
            if (urlargs.stream == null) {
                QH('metadatadiv', '<span style=\"font-family:Arial,Helvetica Neue,Helvetica,sans-serif;font-size:28px\">MeshCentral Session Player</span><br /><br /><span style=color:gray>' + "Trascina e rilascia un file .mcrec o fai clic su \"Apri file ...\"" + '</span>');
            } else {
                QH('metadatadiv', '');
            }
            QV('DeskParent', true);
        ...
        function saveAsWebMfile() {
            var x = '';
            x += addHtmlValue4("Frequenza dei fotogrammi", '<select id=webmframerate style=width:200px><option value=100 selected>' + "10 fotogrammi/sec" + '</option><option value=50>' + "20 fotogrammi/sec" + '</option></select>');
            x += addHtmlValue4("Qualità", '<select id=webmquality style=width:200px><option value=90>' + "90%" + '</option><option value=80>' + "80%" + '</option><option value=60 selected>' + "60%" + '</option><option value=40>' + "40%" + '</option><option value=20>' + "20%" + '</option><option value=10>' + "10%" + '</option></select>');
            setDialogMode(2, "Converti in WebM", 3, saveAsWebMfileEx, x);
        }
        ...
        function openfile() {
            ...
            setDialogMode(2, "Apri il file...", 3, openfileEx, x);
