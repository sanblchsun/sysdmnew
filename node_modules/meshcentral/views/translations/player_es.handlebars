        ...
        <div id="deskarea0">
            <div id="deskarea1" class="areaHead">
                <div class="toright2">
                    ...
                    <input id="ConvertAsWebM" style="display:none" type="button" value="Convierte a WebM" onclick="saveAsWebMfile()">&nbsp;
                </div>
                <div>
                    <input id="OpenFileButton" type="button" value="Abrir Archivo..." onclick="openfile()" style="display:none">
            ...
            <div id="deskarea4" class="areaFoot">
                ...
                <div>
                    &nbsp;
                    <input id="PlayButton" type="button" value="Reproducir" disabled="disabled" onclick="play()">
                    <input id="PauseButton" type="button" value="Pausar" disabled="disabled" onclick="pause()">
                    ...
                    <select id="PlaySpeed" onchange="this.blur();">
                        <option value="4">1/4 Velocidad</option>
                        <option value="2">1/2 Velocidad</option>
                        <option value="1" selected="">Velocidad Normal</option>
                        <option value="0.5">2x Velocidad</option>
                        <option value="0.25">4x Velocidad</option>
                        <option value="0.1">10x Velocidad</option>
                    </select>
    ...
    <script>
        ...
        function start() {
            ...
            browser = (function (agent) {
                switch (true) {
                    case agent.indexOf("borde") > -1: return "MS Edge (EdgeHtml)";
            ...
            if (urlargs.stream != null) {
                QV('metadatadiv', true);
                QH('metadatadiv', "Conectando al servidor...");
        ...
        function processFirstBlock(type, flags, time, data) {
            ...
            var x = '';
            x += addInfo("Tiempo", recFileMetadata.time);
            if (recFileEndTime != 0) { var secs = Math.floor((recFileEndTime - time) / 1000); x += addInfo("Duración", format("{0} segundo{1}", secs, (secs > 1) ? 's' : '')); }
            x += addInfo("Nombre de Usuario", recFileMetadata.username);
            x += addInfo("UserID", recFileMetadata.userid);
            x += addInfo("ID de sesión", recFileMetadata.sessionid);
            if (recFileMetadata.ipaddr1 && recFileMetadata.ipaddr2) { x += addInfo("Direcciones", format("{0} a {1}", recFileMetadata.ipaddr1, recFileMetadata.ipaddr2)); }
            if (recFileMetadata.devicename) { x += addInfo("Nombre del Dispositivo", recFileMetadata.devicename); }
            x += addInfo("ID de Nodo", recFileMetadata.nodeid);
            if (recFileMetadata.protocol) {
                var p = recFileMetadata.protocol;
                if (p == 1) { p = "Terminal MeshCentral"; }
                else if (p == 2) { p = "Escritorio de MeshCentral"; }
                else if (p == 6) { p = "Administrar Powershell"; }
                else if (p == 8) { p = "Shell del Usuario"; }
                else if (p == 9) { p = "Usuario PowerShell"; }
                else if (p == 100) { p = "Intel&reg; AMT WSMAN"; }
                else if (p == 101) { p = "Redirección de Intel&reg; AMT"; }
                else if ((p == 102) || (p == 200 && recFileMetadata.bpp != null)) { p = "Intel&reg; AMT KVM"; }
                else if (p == 200) { p = "MeshMessenger"; }
                x += addInfoNoEsc("Protocolo", p);
            }
            var encQualityStr = "2 bytes por píxel";
            if (recFileMetadata.bpp == 1) { encQualityStr = "1 byte por píxel" }
            if (recFileMetadata.graymode) { if (recFileMetadata.lowcolor) { encQualityStr += ", 16 grises"; } else { encQualityStr += ", 256 grises"; } }
            x += addInfoNoEsc("Calidad de codificación", encQualityStr);

            if (recFileMetadata.indexInterval) {
                recFileIndexBasePtr = recFilePtr;
                x += addInfoNoEsc("Buscando", format("Indexar cada {0} segundos", recFileMetadata.indexInterval));
            ...
            if ((recFileMetadata.protocol == 1) || (recFileMetadata.protocol == 6) || (recFileMetadata.protocol == 8) || (recFileMetadata.protocol == 9)) {
                ...
                recFileProtocol = 1;
                x += '<br /><br /><span style=color:gray>' + "Presiona [espacio] para reproducir / pausar." + '</span>';
                QE('PlayButton', true);
        ...
        function cleanup() {
            ...
            QV('metadatadiv', true);
            if (urlargs.stream == null) {
                QH('metadatadiv', '<span style=\"font-family:Arial,Helvetica Neue,Helvetica,sans-serif;font-size:28px\">MeshCentral Session Player</span><br /><br /><span style=color:gray>' + "Arrastra y suelta un archivo .mcrec o haga clic en \"Abrir Archivo...\"" + '</span>');
            } else {
                QH('metadatadiv', '');
            }
            QV('DeskParent', true);
        ...
        function saveAsWebMfile() {
            var x = '';
            x += addHtmlValue4("Cuadros por segundo", '<select id=webmframerate style=width:200px><option value=100 selected>' + "10 cuadros / seg" + '</option><option value=50>' + "20 cuadros / seg" + '</option></select>');
            x += addHtmlValue4("Calidad", '<select id=webmquality style=width:200px><option value=90>' + "90%" + '</option><option value=80>' + "80%" + '</option><option value=60 selected>' + "60%" + '</option><option value=40>' + "40%" + '</option><option value=20>' + "20%" + '</option><option value=10>' + "10%" + '</option></select>');
            setDialogMode(2, "Convierte a WebM", 3, saveAsWebMfileEx, x);
        }
        ...
        function openfile() {
            ...
            setDialogMode(2, "Abrir Archivo...", 3, openfileEx, x);
