        ...
        <div id="deskarea0">
            <div id="deskarea1" class="areaHead">
                <div class="toright2">
                    ...
                    <input id="ConvertAsWebM" style="display:none" type="button" value="वेबएम में कनवर्ट करें" onclick="saveAsWebMfile()">&nbsp;
                </div>
                <div>
                    <input id="OpenFileButton" type="button" value="खुली फाइल..." onclick="openfile()" style="display:none">
            ...
            <div id="deskarea4" class="areaFoot">
                ...
                <div>
                    &nbsp;
                    <input id="PlayButton" type="button" value="खेल" disabled="disabled" onclick="play()">
                    <input id="PauseButton" type="button" value="ठहराव" disabled="disabled" onclick="pause()">
                    <input id="RestartButton" type="button" value="पुनर्प्रारंभ करें" disabled="disabled" onclick="restart()">
                    <select id="PlaySpeed" onchange="this.blur();">
                        <option value="4">1/4 गति</option>
                        <option value="2">1/2 गति</option>
                        <option value="1" selected="">सामान्य गति</option>
                        <option value="0.5">2x गति</option>
                        <option value="0.25">4x स्पीड</option>
                        <option value="0.1">10x स्पीड</option>
                    </select>
    ...
    <script>
        ...
        function start() {
            ...
            browser = (function (agent) {
                switch (true) {
                    case agent.indexOf("किनारा") > -1: return "एमएस एज (एजएचटीएमएल)";
                    case agent.indexOf("ईडीजी") > -1: return "एमएस एज क्रोमियम";
                    case agent.indexOf("opr") > -1 && !!window.opr: return "ओपेरा";
                    case agent.indexOf("क्रोम") > -1 && !!window.chrome: return "क्रोम";
                    case agent.indexOf("ट्राइडेंट") > -1: return "इंटरनेट एक्स्प्लोरर";
                    case agent.indexOf("फ़ायर्फ़ॉक्स") > -1: return "फ़ायर्फ़ॉक्स";
                    case agent.indexOf("सफारी") > -1: return "सफारी";
                    default: return "अन्य";
                }
            ...
            if (urlargs.stream != null) {
                QV('metadatadiv', true);
                QH('metadatadiv', "सर्वर से कनेक्ट हो रहा है...");
        ...
        function processFirstBlock(type, flags, time, data) {
            ...
            var x = '';
            x += addInfo("समय", recFileMetadata.time);
            if (recFileEndTime != 0) { var secs = Math.floor((recFileEndTime - time) / 1000); x += addInfo("अवधि", format("{0} दूसरा {1}", secs, (secs > 1) ? 's' : '')); }
            x += addInfo("उपयोगकर्ता नाम", recFileMetadata.username);
            x += addInfo("यूज़र आईडी", recFileMetadata.userid);
            x += addInfo("सत्र आईडी", recFileMetadata.sessionid);
            if (recFileMetadata.ipaddr1 && recFileMetadata.ipaddr2) { x += addInfo("पतों", format("{0} से {1}", recFileMetadata.ipaddr1, recFileMetadata.ipaddr2)); }
            if (recFileMetadata.devicename) { x += addInfo("यन्त्र का नाम", recFileMetadata.devicename); }
            x += addInfo("NodeID", recFileMetadata.nodeid);
            if (recFileMetadata.protocol) {
                var p = recFileMetadata.protocol;
                if (p == 1) { p = "मेश्चरल टर्मिनल"; }
                else if (p == 2) { p = "मेश्चरल डेस्कटॉप"; }
                else if (p == 6) { p = "व्यवस्थापक PowerShell"; }
                else if (p == 8) { p = "उपयोगकर्ता शेल"; }
                else if (p == 9) { p = "उपयोगकर्ता पॉवरशेल"; }
                else if (p == 100) { p = "इंटेल&reg; AMT WSMAN"; }
                else if (p == 101) { p = "Intel&reg; AMT पुनर्निर्देशन"; }
                else if ((p == 102) || (p == 200 && recFileMetadata.bpp != null)) { p = "इंटेल&reg; एएमटी केवीएम"; }
                else if (p == 200) { p = "MeshMessenger"; }
                x += addInfoNoEsc("मसविदा बनाना", p);
            }
            var encQualityStr = "2 बाइट-प्रति-पिक्सेल";
            if (recFileMetadata.bpp == 1) { encQualityStr = "1 बाइट-प्रति-पिक्सेल" }
            if (recFileMetadata.graymode) { if (recFileMetadata.lowcolor) { encQualityStr += ", 16 ग्रे"; } else { encQualityStr += ", 256 ग्रे"; } }
            x += addInfoNoEsc("एन्कोडिंग गुणवत्ता", encQualityStr);

            if (recFileMetadata.indexInterval) {
                recFileIndexBasePtr = recFilePtr;
                x += addInfoNoEsc("तलाश", format("प्रत्येक {0} सेकंड अनुक्रमित", recFileMetadata.indexInterval));
            ...
            if ((recFileMetadata.protocol == 1) || (recFileMetadata.protocol == 6) || (recFileMetadata.protocol == 8) || (recFileMetadata.protocol == 9)) {
                ...
                recFileProtocol = 1;
                x += '<br /><br /><span style=color:gray>' + "प्रेस [स्थान] खेलने के लिए / रोकें।" + '</span>';
                QE('PlayButton', true);
        ...
        function processBlockEx(type, flags, time, data, forced) {
            ...
            if ((type == 3) && (recFileProtocol == 102)) {
                var tile = new Image();
                tile.src = "डेटा: छवि / PNG; बेस 64," + btoa(data);
        ...
        function cleanup() {
            ...
            QV('metadatadiv', true);
            if (urlargs.stream == null) {
                QH('metadatadiv', '<span style=\"font-family:Arial,Helvetica Neue,Helvetica,sans-serif;font-size:28px\">MeshCentral Session Player</span><br /><br /><span style=color:gray>' + "एक .mcrec फ़ाइल को खींचें और छोड़ दें या \"फ़ाइल खोलें ... \" पर क्लिक करें" + '</span>');
            } else {
                QH('metadatadiv', '');
            }
            QV('DeskParent', true);
        ...
        function saveAsWebMfile() {
            var x = '';
            x += addHtmlValue4("फ्रेम रेट", '<select id=webmframerate style=width:200px><option value=100 selected>' + "१० फ्रेम/सेकंड" + '</option><option value=50>' + "20 फ्रेम/सेकंड" + '</option></select>');
            x += addHtmlValue4("गुणवत्ता", '<select id=webmquality style=width:200px><option value=90>' + "90%" + '</option><option value=80>' + "80%" + '</option><option value=60 selected>' + "60%" + '</option><option value=40>' + "40%" + '</option><option value=20>' + "20%" + '</option><option value=10>' + "10%" + '</option></select>');
            setDialogMode(2, "वेबएम में कनवर्ट करें", 3, saveAsWebMfileEx, x);
        }
        ...
        function openfile() {
            ...
            setDialogMode(2, "खुली फाइल...", 3, openfileEx, x);
