        ...
        <div id="deskarea0">
            <div id="deskarea1" class="areaHead">
                <div class="toright2">
                    ...
                    <input id="ConvertAsWebM" style="display:none" type="button" value="Převést na WebM" onclick="saveAsWebMfile()">&nbsp;
                </div>
                <div>
                    <input id="OpenFileButton" type="button" value="Otevřít soubor…" onclick="openfile()" style="display:none">
            ...
            <div id="deskarea4" class="areaFoot">
                ...
                <div>
                    ...
                    <input id="PauseButton" type="button" value="Pozastavit" disabled="disabled" onclick="pause()">
    ...
    <script>
        ...
        function start() {
            ...
            browser = (function (agent) {
                switch (true) {
                    case agent.indexOf("okraj") > -1: return "MS Edge (EdgeHtml)";
                    case agent.indexOf("hrana") > -1: return "MS Edge Chromium";
                    case agent.indexOf("opr") > -1 && !!window.opr: return "opera";
                    case agent.indexOf("Chrome") > -1 && !!window.chrome: return "Chrome";
                    case agent.indexOf("trojzubec") > -1: return "internet Explorer";
            ...
            if (urlargs.stream != null) {
                QV('metadatadiv', true);
                QH('metadatadiv', "Připojení k serveru...");
        ...
        function processFirstBlock(type, flags, time, data) {
            ...
            if (recFileEndTime != 0) { var secs = Math.floor((recFileEndTime - time) / 1000); x += addInfo("Délka trvání", format("{0} sekund{1}", secs, (secs > 1) ? 's' : '')); }
            x += addInfo("Uživatelské jméno", recFileMetadata.username);
            x += addInfo("Identif. uživatele", recFileMetadata.userid);
            x += addInfo("ID relace", recFileMetadata.sessionid);
            ...
            x += addInfo("Identif. uzlu", recFileMetadata.nodeid);
            if (recFileMetadata.protocol) {
                var p = recFileMetadata.protocol;
                if (p == 1) { p = "MeshCentral terminal"; }
                else if (p == 2) { p = "Plocha MeshCentral"; }
                else if (p == 6) { p = "PowerShell správce"; }
                else if (p == 8) { p = "Shell uživatele"; }
                else if (p == 9) { p = "PowerShell uživatele"; }
            ...
            var encQualityStr = "2 bajty na pixel";
            if (recFileMetadata.bpp == 1) { encQualityStr = "1 bajt na pixel" }
            if (recFileMetadata.graymode) { if (recFileMetadata.lowcolor) { encQualityStr += ", 16 šedá"; } else { encQualityStr += ", 256 šedá"; } }
            x += addInfoNoEsc("Kvalita kódování", encQualityStr);

            if (recFileMetadata.indexInterval) {
                recFileIndexBasePtr = recFilePtr;
                x += addInfoNoEsc("Hledám", format("Indexováno každých {0} sekund", recFileMetadata.indexInterval));
            ...
            if ((recFileMetadata.protocol == 1) || (recFileMetadata.protocol == 6) || (recFileMetadata.protocol == 8) || (recFileMetadata.protocol == 9)) {
                ...
                recFileProtocol = 1;
                x += '<br /><br /><span style=color:gray>' + "Pro pokračování/pozastavení stiskněte [mezerník]." + '</span>';
                QE('PlayButton', true);
        ...
        function cleanup() {
            ...
            QV('metadatadiv', true);
            if (urlargs.stream == null) {
                QH('metadatadiv', '<span style=\"font-family:Arial,Helvetica Neue,Helvetica,sans-serif;font-size:28px\">MeshCentral Session Player</span><br /><br /><span style=color:gray>' + "Přetáhněte soubor .mcrec nebo klikněte na „Otevřít soubor…“" + '</span>');
            } else {
                QH('metadatadiv', '');
            }
            QV('DeskParent', true);
        ...
        function saveAsWebMfile() {
            var x = '';
            x += addHtmlValue4("Snímková frekvence", '<select id=webmframerate style=width:200px><option value=100 selected>' + "10 snímků/s" + '</option><option value=50>' + "20 snímků/s" + '</option></select>');
            x += addHtmlValue4("Kvalita", '<select id=webmquality style=width:200px><option value=90>' + "90%" + '</option><option value=80>' + "80%" + '</option><option value=60 selected>' + "60%" + '</option><option value=40>' + "40%" + '</option><option value=20>' + "20%" + '</option><option value=10>' + "10%" + '</option></select>');
            setDialogMode(2, "Převést na WebM", 3, saveAsWebMfileEx, x);
        }
