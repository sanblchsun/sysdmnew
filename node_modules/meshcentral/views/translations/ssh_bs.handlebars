<!DOCTYPE html><html dir="ltr" xmlns="http://www.w3.org/1999/xhtml"><head lang="en">
    ...
    <div id="p11" class="noselect" style="overflow:hidden">
        <div id="deskarea0" style="position:relative">
            <div id="deskarea1" class="areaHead">
                ...
                <div>
                    <input id="ConnectButton" style="display:none" type="button" value="Povežite se" onclick="connectButton()">
                    <input id="DisconnectButton" type="button" value="Prekini vezu" onclick="connectButton()">
    ...
    <script>
        ...
        var StatusStrs = ["Disconnected", "Povezivanje...", "Postaviti...", "Povezano"];
        var state = 0;
        var socket = null;
        var user = '';
        var pass = '';
        ...
        function sshTunnelAuthDialog(j, func) {
            var x = '';
            if (j.askkeypass) {
                x += addHtmlValue("Autentifikacija", '<select id=dp2authmethod style=width:230px onchange=sshAuthUpdate(event)><option value=3 selected>' + "Pohranjeni ključ" + '</option><option value=1>' + "Korisnicko ime lozinka" + '</option><option value=2>' + "Korisničko ime i ključ" + '</option></select>');
            } else {
                x += addHtmlValue("Autentifikacija", '<select id=dp2authmethod style=width:230px onchange=sshAuthUpdate(event)><option value=1 selected>' + "Korisnicko ime lozinka" + '</option><option value=2>' + "Korisničko ime i ključ" + '</option></select>');
            }
            x += '<div id=d2userauth style=display:none>';
            x += addHtmlValue("Korisničko ime", '<input id=dp2user style=width:230px maxlength=64 autocomplete=off onkeyup=sshAuthUpdate(event) />');
            x += '</div>';
            x += '<div id=d2passauth style=display:none>';
            x += addHtmlValue("Lozinka", '<input type=password id=dp2pass style=width:230px maxlength=64 autocomplete=off onkeyup=sshAuthUpdate(event) />');
            if ((features & 1) == 0) { x += addHtmlValue('', '<label><input id=dp2keep type=checkbox>' + "Zapamtite akreditive" + '</label>'); }
            x += '</div><div id=d2keyauth style=display:none>';
            x += addHtmlValue("Key File", '<input type=file id=dp2key style=width:230px maxlength=64 autocomplete=off onchange=sshAuthUpdate(event) />' + '<div id=d2badkey style=font-size:x-small>' + "Datoteka ključa mora biti u OpenSSH formatu." + '</div>');
            x += addHtmlValue("Ključna lozinka", '<input type=password id=dp2keypass style=width:230px maxlength=64 autocomplete=off onkeyup=sshAuthUpdate(event) />');
            if ((features & 1) == 0) {
                x += addHtmlValue('', '<label><input id=dp2keep1 type=checkbox onchange=sshAuthUpdate(event)>' + "Zapamtite korisnika i ključ" + '</label>');
                x += addHtmlValue('', '<label><input id=dp2keep2 type=checkbox>' + "Zapamti lozinku" + '</label>');
            }
            x += '</div>';
            if (j.askkeypass) {
                x += '<div id=d2keyauth2 style=display:none>';
                x += addHtmlValue("Lozinka", '<input type=password id=dp2keypass2 style=width:230px maxlength=64 autocomplete=off onkeyup=sshAuthUpdate(event) />');
                x += '</div>';
            }
            ...
            socket.onmessage = function (data) {
                ...
                if (data.data[0] == '{') {
                    ...
                    switch (json.action) {
                        ...
                        case 'autherror': { setDialogMode(2, "Autentifikacija", 1, null, "Nije moguće izvršiti autentifikaciju."); break; }
                        case 'sessionerror': { setDialogMode(2, "Sjednica", 1, null, "Sesija je istekla."); break; }
                        case 'sessiontimeout': { setDialogMode(2, "Sjednica", 1, null, "Vremensko ograničenje sesije."); break; }
                    }
                } else if (data.data[0] == '~') {
