        ...
        <div id="deskarea0">
            <div id="deskarea1" class="areaHead">
                <div class="toright2">
                    ...
                    <input id="ConvertAsWebM" style="display:none" type="button" value="WebM으로 변환" onclick="saveAsWebMfile()">&nbsp;
                </div>
                <div>
                    <input id="OpenFileButton" type="button" value="파일 열기 ..." onclick="openfile()" style="display:none">
            ...
            <div id="deskarea4" class="areaFoot">
                ...
                <div>
                    &nbsp;
                    <input id="PlayButton" type="button" value="재생" disabled="disabled" onclick="play()">
                    <input id="PauseButton" type="button" value="중지" disabled="disabled" onclick="pause()">
                    <input id="RestartButton" type="button" value="재시작" disabled="disabled" onclick="restart()">
                    <select id="PlaySpeed" onchange="this.blur();">
                        <option value="4">1/4배속</option>
                        <option value="2">1/2배속</option>
                        <option value="1" selected="">정상 속도</option>
                        <option value="0.5">2배속</option>
                        <option value="0.25">4배속</option>
                        <option value="0.1">10배속</option>
                    </select>
    ...
    <script>
        ...
        function start() {
            ...
            browser = (function (agent) {
                switch (true) {
                    case agent.indexOf("가장자리") > -1: return "MS 에지(EdgeHtml)";
                    case agent.indexOf("가장자리") > -1: return "MS 엣지 크롬";
                    case agent.indexOf("작전") > -1 && !!window.opr: return "오페라";
                    case agent.indexOf("Chrome") > -1 && !!window.chrome: return "Chrome";
                    case agent.indexOf("삼지창") > -1: return "인터넷 익스플로러";
                    case agent.indexOf("파이어폭스") > -1: return "파이어폭스";
                    case agent.indexOf("원정 여행") > -1: return "원정 여행";
                    default: return "다른";
                }
        ...
        function processFirstBlock(type, flags, time, data) {
            ...
            var x = '';
            x += addInfo("시간", recFileMetadata.time);
            if (recFileEndTime != 0) { var secs = Math.floor((recFileEndTime - time) / 1000); x += addInfo("지속", format("{0} 초 {1}", secs, (secs > 1) ? 's' : '')); }
            x += addInfo("사용자 이름", recFileMetadata.username);
            x += addInfo("사용자 ID", recFileMetadata.userid);
            x += addInfo("세션 ID", recFileMetadata.sessionid);
            if (recFileMetadata.ipaddr1 && recFileMetadata.ipaddr2) { x += addInfo("주소들", format("{0}에서 {1}", recFileMetadata.ipaddr1, recFileMetadata.ipaddr2)); }
            if (recFileMetadata.devicename) { x += addInfo("장치 이름", recFileMetadata.devicename); }
            x += addInfo("기기 ID", recFileMetadata.nodeid);
            if (recFileMetadata.protocol) {
                var p = recFileMetadata.protocol;
                if (p == 1) { p = "MeshCentral 터미널"; }
                else if (p == 2) { p = "MeshCentral 데스크탑"; }
                else if (p == 6) { p = "PowerShell 관리자"; }
                else if (p == 8) { p = "사용자 쉘"; }
                else if (p == 9) { p = "사용자 PowerShell"; }
                else if (p == 100) { p = "Intel&reg; AMT WSMAN"; }
                else if (p == 101) { p = "Intel&reg; AMT 리디렉션"; }
                else if ((p == 102) || (p == 200 && recFileMetadata.bpp != null)) { p = "인텔 &reg; AMT KVM"; }
                else if (p == 200) { p = "메시 메신저"; }
                x += addInfoNoEsc("프로토콜", p);
            }
            var encQualityStr = "픽셀당 2바이트";
            if (recFileMetadata.bpp == 1) { encQualityStr = "픽셀당 1바이트" }
            if (recFileMetadata.graymode) { if (recFileMetadata.lowcolor) { encQualityStr += ", 16 그레이"; } else { encQualityStr += ", 256 그레이"; } }
            x += addInfoNoEsc("인코딩 품질", encQualityStr);

            if (recFileMetadata.indexInterval) {
                recFileIndexBasePtr = recFilePtr;
                x += addInfoNoEsc("찾는 중", format("{0}초마다 색인 생성", recFileMetadata.indexInterval));
            ...
            if ((recFileMetadata.protocol == 1) || (recFileMetadata.protocol == 6) || (recFileMetadata.protocol == 8) || (recFileMetadata.protocol == 9)) {
                ...
                recFileProtocol = 1;
                x += '<br /><br /><span style=color:gray>' + "재생 / 일시정지하려면 [space]를 누르십시오." + '</span>';
                QE('PlayButton', true);
        ...
        function processBlockEx(type, flags, time, data, forced) {
            ...
            if ((type == 3) && (recFileProtocol == 102)) {
                var tile = new Image();
                tile.src = "데이터 : 이미지 / png; base64," + btoa(data);
        ...
        function cleanup() {
            ...
            QV('metadatadiv', true);
            if (urlargs.stream == null) {
                QH('metadatadiv', '<span style=\"font-family:Arial,Helvetica Neue,Helvetica,sans-serif;font-size:28px\">MeshCentral Session Player</span><br /><br /><span style=color:gray>' + ".mcrec 파일을 끌어다 놓거나 \"파일 열기 ... \"를 클릭하십시오." + '</span>');
            } else {
                QH('metadatadiv', '');
            }
            QV('DeskParent', true);
        ...
        function saveAsWebMfile() {
            var x = '';
            x += addHtmlValue4("프레임 속도", '<select id=webmframerate style=width:200px><option value=100 selected>' + "10프레임/초" + '</option><option value=50>' + "20프레임/초" + '</option></select>');
            x += addHtmlValue4("품질", '<select id=webmquality style=width:200px><option value=90>' + "90%" + '</option><option value=80>' + "80%" + '</option><option value=60 selected>' + "60%" + '</option><option value=40>' + "40%" + '</option><option value=20>' + "20%" + '</option><option value=10>' + "10%" + '</option></select>');
            setDialogMode(2, "WebM으로 변환", 3, saveAsWebMfileEx, x);
        }
        ...
        function openfile() {
            ...
            setDialogMode(2, "파일 열기 ...", 3, openfileEx, x);
