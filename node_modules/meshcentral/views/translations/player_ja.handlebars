        ...
        <div id="deskarea0">
            <div id="deskarea1" class="areaHead">
                <div class="toright2">
                    ...
                    <input id="ConvertAsWebM" style="display:none" type="button" value="WebMに変換する" onclick="saveAsWebMfile()">&nbsp;
                </div>
                <div>
                    <input id="OpenFileButton" type="button" value="ファイルを開く..." onclick="openfile()" style="display:none">
            ...
            <div id="deskarea4" class="areaFoot">
                ...
                <div>
                    &nbsp;
                    <input id="PlayButton" type="button" value="再生" disabled="disabled" onclick="play()">
                    ...
                    <select id="PlaySpeed" onchange="this.blur();">
                        <option value="4">1/4倍速</option>
                        <option value="2">1/2倍速</option>
    ...
    <script>
        ...
        function start() {
            ...
            browser = (function (agent) {
                switch (true) {
                    case agent.indexOf("縁") > -1: return "MS Edge（EdgeHtml）";
                    case agent.indexOf("エッジ") > -1: return "MS Edge Chromium";
                    case agent.indexOf("opr") > -1 && !!window.opr: return "オペラ";
                    case agent.indexOf("Chrome") > -1 && !!window.chrome: return "Chrome";
                    case agent.indexOf("トライデント") > -1: return "インターネットエクスプローラ";
                    case agent.indexOf("Firefox") > -1: return "Firefox";
                    case agent.indexOf("サファリ") > -1: return "サファリ";
                    default: return "他の";
                }
        ...
        function processFirstBlock(type, flags, time, data) {
            ...
            if (recFileMetadata.protocol) {
                ...
                else if (p == 6) { p = "管理者PowerShell"; }
                else if (p == 8) { p = "ユーザーシェル"; }
                else if (p == 9) { p = "ユーザーPowerShell"; }
                else if (p == 100) { p = "Intel&reg; AMT WSMAN"; }
                else if (p == 101) { p = "Intel&reg; AMTリダイレクト"; }
                else if ((p == 102) || (p == 200 && recFileMetadata.bpp != null)) { p = "インテル&reg;AMT KVM"; }
                else if (p == 200) { p = "MeshMessenger"; }
                x += addInfoNoEsc("プロトコル", p);
            }
            var encQualityStr = "2バイト/ピクセル";
            if (recFileMetadata.bpp == 1) { encQualityStr = "1ピクセル毎に1バイト" }
            if (recFileMetadata.graymode) { if (recFileMetadata.lowcolor) { encQualityStr += ", 16 グレー"; } else { encQualityStr += ", 256 グレー"; } }
            x += addInfoNoEsc("エンコード品質", encQualityStr);

            if (recFileMetadata.indexInterval) {
                recFileIndexBasePtr = recFilePtr;
                x += addInfoNoEsc("探している", format("{0}秒ごとにインデックス付けされます", recFileMetadata.indexInterval));
        ...
        function saveAsWebMfile() {
            var x = '';
            x += addHtmlValue4("フレームレート", '<select id=webmframerate style=width:200px><option value=100 selected>' + "10フレーム/秒" + '</option><option value=50>' + "20フレーム/秒" + '</option></select>');
            ...
            setDialogMode(2, "WebMに変換する", 3, saveAsWebMfileEx, x);
        }
